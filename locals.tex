...
\begin{worddef}[b:]{23}[4]{\brace:}[brace-colon][x:enhanced-locals]
\interpret
	Interpretation semantics for this word are undefined.

\compile
	\stack{i*x "<spaces>ccc:\}"}{}
	...
	\item \word{b:} \ldots\ \verb":}" is used more than once in a word.
	\end{itemize}
...
\see \xref[2.2 Notation]{notations},
	...
	\begin{rationale}
		In \xref{rat:local} of ANS Forth '94 the TC identifies the significant
		...
		they provided a way to define locals (\xref{local:LOCAL}) and a
		method of parsing them (\xref{local:LOCALS}). In the hope that a
		common practice would emerge.

		Common practice has since emerged. Most implementations that provide
		\word{LOCAL} and \word{LOCALS} also provide some form of the
		\{ \ldots\ \} notation; however, this notation conflicts with other
		systems. The \word{b:} \ldots\ \verb":}" notation is a compromise to
