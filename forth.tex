...
% \see				Deprecated in favour of also
%
% \walso[<change>]{<label>}	Add word reference <label> to see also list
% \ralso[<change>]{<label>}	Add rational reference <label> to see also list
% \ialso[<change>]{<label>}	Add implementation reference <label> to see also
% \talso[<change>]{<label>}	Add testing reference <label> to see also list
% \xalso[<change>]{<label>}	Add <label> to see also list with <change> control
%
...
\newcommand{\see}{\samepage\item[See]\@latex@warning{see has been deprecated in favour of also}}
...
%
% \ralso[+/-]{label}		- Reference the rationale for the label
% \ialso[+/-]{label}		- Reference the implementation of the label
% \talso[+/-]{label}		- Reference the testing for the label
% \walso[+/-]{label}		- Reference to a word defintion with the label
% \xalso[+/-]{label}		- Normal cross reference
%
% The optional [+/-] argument will mark the references as being added (+)
% or removed (-) and place the change bars the ed<year> marginal note.
%
% The code references (\ralso, \ialso, \talso and \walso) all format the
% name of the label in teletype, while \xalso does not.

\newcommand{\ralso}{\also@c{rat:}}
\newcommand{\ialso}{\also@c{imp:}}	
\newcommand{\talso}{\also@c{test:}}
\newcommand{\walso}{\also@c{}}
\newcommand{\xalso}[2][!]{\also@v{#1}{@}{#2}}

\newcommand{\also@c}[1]{\edef\temp@a{#1}\also@d}
\newcommand{\also@d}[2][!]{\also@v{#1}{!}{\temp@a#2}}
\newcommand{\also@v}[3]{%
	\expandafter\ifx\csname r@#3\endcsname\relax\else%
		\edef\also@list{\also@list,{#1}{#2}{#3}}
	\fi%
}

% The \alsolist macro is designed to process the \also@list and render the
% arguments, it is a debug tool used to check we can process the list
% correctly.
...
% it works and does not take up too much time.
...
\newcommand{\alsoevaluate}{%
	...
	\loop%
		\@for\temp@a:=\also@list\do{%
			\ifx\temp@a\empty\else%
				\expandafter\also@eval\temp@a%
			\fi%
		}%
		\stepcounter{also}%
	\ifnum\c@also<1000\repeat%
	\ifalso@first\else.\fi
}

% Process the three parameters that form the entry.
% #1 - the cange control is copied into the \temp@b macro.
% #2 - the formatting control is copied into the \temp@c macro.
% #3 - label for the item being evaluated,
%		this is expanded and passed to \also@ref for further processing.
\newcommand{\also@eval}[3]{% <change> <format> <label>
	\let\temp@b=\relax%
	\let\temp@c=\relax%
	\ifx+#1\let\temp@b=\uline\fi%
	\ifx-#1\let\temp@b=\sout\fi%
	\ifx!#2\let\temp@c=\texttt\fi%
	\expandafter\expandafter\expandafter\also@ref\csname r@#3\endcsname%
}

% If the label is on the current page it will generate the cross reference
% for the label.
% #1 - Section number for the item.
% #2 - Page the section is defined on.
% #3 - Title of the definition.
% #4 - The hyper reference target
%
% If this is the first item to be generated it will add the "See also" label
% otherwise add a comma to the output.  If a change control (\temp@b) was
% given it will add the change bars and the ed<year> marginal note.  It uses
% use \temp@c macro to format the name of the link.
\newcommand{\also@ref}[4]{% <number> <page> <title> <target>
	\expandafter\ifnum#2=\c@also%
		\ifalso@first%
			\samepage\item[See also]
			\also@firstfalse
		\else, \fi
		%
		\ifx\temp@b\relax\else\cbstart\fi%
		\temp@b{%
			\hyperref{}{}{#4}{#1} \linebreak[3]
			\hyperref{}{}{#4}{\temp@c{#3}} \linebreak[3]
			(page \hyperref{}{}{#4}{#2})%
		}%
		\ifx\temp@b\relax\else\cbend\patch{ed\shortyear{\the\year}}\fi%
	\fi%
}
