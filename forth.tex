...
% The following disables the underfull box error reports

\hbadness=99999
\vbadness=99999

% The following removes the overfull box error reports
...
% \see				See references (not to be used with also macros)
%
% \walso[+/-][<prop>]{<label>}	Add word reference <label> to see also list
% \ralso[+/-][<prop>]{<label>}	Add rational reference <label> to see also list
% \ialso[+/-][<prop>]{<label>}	Add implementation reference <label> to see also list
% \talso[+/-][<prop>]{<label>}	Add testing reference <label> to see also list
% \xalso[+/-][<prop>]{<label>}	Add <label> to see also list
%
...
\newcommand{\word@def@end}{%
	...
	\def\@noitemerr{\word@error{Something wrong here --- perhaps a
		missing \string\item}}
	%
	% Add Rational, Implementation and Testing to the see also list
	%
	\let\also@error=\also@ignore
	\ralso{\word@list:\word@label}
	\ialso{\word@list:\word@label}
	\talso{\word@list:\word@label}
	\let\also@error=\also@report
	%\item[debug] \alsolist
	%
	% Generate the see also list
	%
	\alsoevaluate
	%
...
\ifinline
	...
	\newcommand{\inline@label}[2]{% {<space>}{<annex>}
		...
		% \also@code{#1:}{1}{\word@list:\word@label}
	}
...
\else
	...
	% the auxiliary file.  This would be the place to add the environment
	% to the see also list for the definiton.  However, to do so would
	% require we use \gdef in \also@v, which causes LaTeX to run out of
	% memory.  Thus we we limited to useing \edef which means we need to
	% run the \<x>also words in the wordef environment.

	% \newcommand{\prerationale}{\ralso{\word@list:\word@label}}
	% \newcommand{\pretesting}{\talso{\word@list:\word@label}}
	% \newcommand{\preimplement}{\ialso{\word@list:\word@label}}
...
\newcommand{\proposal}[1]{%
	\ifrelease\else%
		\vspace*{-3ex}\hfill\textsf{\small #1}\\[-4ex]
	\fi
}
...
\newcommand{\word@xref}[4]{% {number}{page}{name}{link}
	\expandafter\ifx#1\empty\else\hyperref{}{}{#4}{#1}\fi
	\hyperref{}{}{#4}{#3}%
}
...
% ========== See also list ==========
%
% This works by adding each also request to a list (\also@list) which is
% reset at the start of each word defintiion and processed and the end of
% each definition.

\def\also@list{}
\newcommand{\also@init}{\gdef\also@list{}}

% When processing the automatic referenes for the current word the error
% reporting for missing references is surpress (\also@ignore).  However,
% for user added entries we render a reference error (?? <label>) when the
% referenced label is not defined (\also@report).
% This is controlled by the \also@error macro.

\newcommand{\also@report}[1]{\textbf{[?? #1]}}
\newcommand{\also@ignore}[1]{}
\let\also@error=\also@report

% To add a reference to the list use one of the following:
%
% \ralso[+/-][prop]{label}	- Reference the rationale for the label
% \ialso[+/-][prop]{label}	- Reference the implementation of the label
% \talso[+/-][prop]{label}	- Reference the testing for the label
% \walso[+/-][prop]{label}	- Reference to a word defintion with the label
% \xalso[+/-][prop]{label}	- Normal cross reference
%
% The optional [+/-] argument will mark the references as being a change
% + for addition and - for removal.  If given then the second optional
% argument (prop) is the name of the proposal, ed<year> will be used if no
% prosal name is given.
%
% The code references (\ralso, \ialso, \talso and \walso) all format the
% title of the label in teletype (1), while \xalso uses roman (0).

\newcommand{\ralso}{\also@code{rat:}{1}}
\newcommand{\ialso}{\also@code{imp:}{1}}
\newcommand{\talso}{\also@code{test:}{1}}
\newcommand{\walso}{\also@code{}{1}}
\newcommand{\xalso}{\also@code{}{0}}

\newcommand{\also@code}[2]{
	\edef\also@s{#1}
	\edef\also@t{#2}
	\also@change
}

\newcommand{\also@change}[1][!]{
	\edef\also@c{#1}
	\also@data
}
	
\newcommand{\also@data}[2][ed\shortyear{\the\year}]{
	\expandafter\ifx\csname r@\also@s#2\endcsname\relax
		\also@error{\also@s#2}
	\else
		\expandafter\expandafter\expandafter\also@value\csname r@\also@s#2\endcsname{\also@c}{#1}{\also@t}
	\fi
}

\newcommand{\also@value}[8]{
	\edef\also@list{\also@list,{#2}{#4}{#1}{#3}{#6}{#7}{#8}}
}

% The \alsolist macro is designed to process the list of see also references
% rendering each entry, it is a debug tool used to check we can process the
% list correctly.

\newcommand{\alsolist}{%
	\@for\temp@a:=\also@list\do{
		\ifx\temp@a\empty\else
			\expandafter\also@format\temp@a
		\fi
	}
}
\newcommand{\also@format}[7]{[#1][#2][#3][#4][#5][#6][#7]\\}

% \alsoevaluate is called by the end definition code to go though the
% \also@list one page at a time rendering all defintions on that page
% by calling \also@eval for each entry in the list.
% I know this is a very poor method of filtering/sorting the list but
% it works and does not take up too much time.  Note the magic number
% representing the total number of pages in the document.

\newcounter{also}
\newif\ifalso@first\also@firsttrue
\newcommand{\alsoevaluate}{
	\setcounter{also}{1}
	\also@firsttrue
	\loop
		\@for\tmp@a:=\also@list\do{%
			\ifx\tmp@a\empty\else%
				\expandafter\also@eval\tmp@a%
			\fi%
		}%
		\stepcounter{also}%
	\ifnum\c@also<1000\repeat% <=== magic number - total number of pages
	\ifalso@first\else\linebreak[0].\fi
}

% Process the each entry from the list.
% #1 - page number
% #2 - hyper link
% #3 - section number
% #4 - title
% #5 - change control + (addition), - (removeal), ! (no-change)
% #6 - propoal
% #7 - format of title 1 (code), 0 (roman)
\newcommand{\also@eval}[7]{%
	\expandafter\ifnum#1=\c@also
		%
		% Title format
		%
		\let\also@t=\textrm%
		\ifx1#7\let\also@t\texttt\fi%
		%
		% Change formatting
		%
		\let\also@c=\relax%
		\ifx+#5\let\also@c\uline\fi%
		\ifx-#5\let\also@c\sout\fi%
		%
		% If first output, add See also else add comma
		\ifalso@first%
			\also@firstfalse%
			\item[See also]%
		\else\linebreak[0], \linebreak[3]
		\fi%
		%
		% Change bar start
		%
		\ifx\also@c\relax\else\cbstart\patch{#6}\fi%
		%
		% Render reference
		%
		\def\also@n{#3}
		\ifx\also@n\empty\relax\else\def\also@n{\hyperref{}{}{#2}{#3}\space}\fi
		\also@c{%
			\also@n
			\hyperref{}{}{#2}{\also@t{#4}}
			(page \hyperref{}{}{#2}{#1})\linebreak[0]%
		}%
		%
		% Change bar end
		%
		\ifx\also@c\relax\else\cbend\fi%
	\fi
}

% helper macros used for taking the first two digits of off the current year
...
\renewcommand{\section}{\@startsection
	{section}{1}{0pt}{1pt}{2pt}
	{\normalfont\Large\sffamily\bfseries}
}
\renewcommand{\subsection}{\@startsection
	{subsection}{2}{0pt}{5pt}{1pt}
	{\normalfont\large\sffamily\bfseries}
}
\renewcommand{\subsubsection}{\@startsection
	{subsubsection}{3}{0pt}{3pt}{1pt}
...
\newcommand{\patch}[1]{% {<name>}
	...
	\else
		...
		\marginpar{%
			\textsf{\footnotesize\colorbox{red!20}{#1}}%
			\tiny\color{white}c:b
		}
...
% The LaTeX to HTML converter occasionally needs a helping hand.
% The \html{} command allows me to embed HTML that is copied
% directly into the HTML output untouched.  But is ignored by
% LaTeX.
\newcommand{\html}[1]{}
...
\begin{document}
%
% Because some people are unable to handle the concept of sheet numbers being
% different to page numbers we have to forgo the conventional book practice of
% of front, main and back matter, and set the entire document as main matter,
% with the title page being sheet/page 1.  This way the page and sheet numbers
% are syncronised.  When these users view the pdf with an inferior viewer the
% the goto sheet number function, which they think is a goto page number
% function, will work in the way they expect.
%
\cleardoublepage
\mainmatter
\setcounter{page}{1}
