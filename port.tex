...
A primary goal of \replace{ed13}{the this Standard is to}{ Forth 94 was to} enable a programmer to write Forth programs that work on
a wide variety of machines\place{ed13}{ , Forth-\snapshot{} continues this practise}.
This \place{ed13}{goal} is accomplished by allowing some key Forth terms to be
...
\word{!} \remove{ed13}{(store)} do the same things (duplicate the top cell on the stack
...
The same code on a 16-bit big\place{ed13}{-}endian Forth would produce the
