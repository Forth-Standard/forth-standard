...
\begin{worddef}{0825}{BUFFER:}[buffer-colon]
...
\begin{worddef}{1177}{DEFER@}[defer-fetch]
...
\see \rref{core:WITHIN}{}.
	...
	\begin{rationale} % A.6.2.2440 WITHIN
		...
		\begin{quote}\ttfamily
			...
			\word{;}
		\end{quote}
		Assume \remove{x:2-complement}{two's-complement arithmetic on} a 16-bit machine, and
		...
		\replace{x:2-complement}{For two's-complement machines that ignore arithmetic overflow
		(most machines), the following implementation works in all
		cases:}{The following implementation works in all cases:}
		\begin{quote}\ttfamily
			\word{:} \word{WITHIN} \word{p} test low high -{}- flag ) \\
	...
	\begin{implement}\cbstart\patch{x:2-complement}
		\uline{\word{:} \word{WITHIN} \word{p} test low high -{}- flag )} \\
		\tab \uline{\word{OVER} \word{-} \word{toR} \word{-} \word{Rfrom} \word{Uless}} \\
		\uline{\word{;}}\cbend
	\end{implement}
\end{worddef}
...
\note \place{ed14}{This word is obsolescent and is included as a concession to
	existing implementations.}
