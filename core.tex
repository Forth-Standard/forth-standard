...
\see \rref{core:ACCEPT}{}.
	...
	\remove{x:legacy}{or \word{EXPECT}}.
	...
	\remove{x:legacy}{%
		...
		behavior of \word{EXPECT}. However, because}
	\place{x:legacy}{Because} external system
...
\item \stack{}{}
	...
	name \place{x:synonym}{or if it was defined as a \word[tools]{SYNONYM}.}

\see \place{x:synonym}{\wref{tools:SYNONYM},}
	\rref{core:IMMEDIATE}{},
	\xref[D.6.7 Immediacy]{diff:immediate}.
...
\see \xref[3.4.1 Parsing]{usage:parsing},
	\wref{core:Cq}{C"},
	\wref{file:Sq}{S"},
	\place{x:escaped-strings}{\wref{core:Seq}{},}
	\rref{core:Sq}{}.
...
\see \rref{core:SOURCE}{}.
	...
	\begin{rationale} % A.6.1.2216 SOURCE
		...
		\remove{x:legacy}{, declaring \word{TIB} and \word{numTIB} obsolescent}.
...
\item \stack{char "<chars>ccc<char>"}{c-addr}
	...
	\remove{x:legacy}{A space, not included in the length, follows
	the string.}
...
\see \xref[3.3.3.6 Other transient regions]{usage:transient},
	...
	\remove{x:legacy}{%
		As a concession to the obsolescent word \word{CONVERT},
...
\item \stack{}{a-addr}\strike{2.5}{65}
...
\sout{	The function of \word{numTIB} has been superseded by
		\word{SOURCE}.}
	\end{rationale}
\end{worddef*}
...
\item \stack{ud_1 c-addr_1}{ud_2 c-addr_2}\strike{2.5}{130}

	\param{ud_2}\strike{2.5}{15}
	\sout{is the result of converting the characters within
	the text beginning at the first character after}
	\param{c-addr_1}\strike{2.5}{35}
	\sout{into digits, using the number in \word{BASE}, and adding each
	digit to}
	\param{ud_1}\strike{2.5}{17}
	\sout{after multiplying}
	\param{ud_1}\strike{2.5}{17}
	\sout{by the
	number in \word{BASE}. Conversion continues until a character
	that is not convertible is encountered.}
	\param{c-addr_2}\strike{2.5}{35}
	\sout{is
	the location of the first unconverted character. An ambiguous
	condition exists if}
	\param{ud_2}\strike{2.5}{17} \sout{overflows.}
...
\see \xref[3.2.1.2 Digit conversion]{usage:digits}.\strike{2.5}{105}

	\begin{rationale} % A.6.2.0970 CONVERT
	\sout{\word{CONVERT} may be defined as follows:}

		\tab \sout{\word{:} \word{CONVERT}~
...
\item \stack{c-addr +n}{}\strike{2.5}{80}
...
\note\sout{%
	...
	\sout{Specification of positive integer counts (\param{+n}) for
...
\note\sout{%
	...
	\sout{The function of \word{QUERY} may be performed with \word{ACCEPT}
		and \word{EVALUATE}.}
	\end{rationale}
\end{worddef*}
...
\see \wref{block:REFILL}{REFILL},
	...
	\remove{x:legacy}{This word is a useful generalization of \word{QUERY}.
		Re-defining \word{QUERY} to meet this specification would
		have broken existing code.}

		\word{REFILL} is designed to behave
		reasonably for all possible input sources. If the input source
		is coming from the user, \remove{x:legacy}{as with \word{QUERY},} \word{REFILL}
...
\see \place{x:n-to-r}{\wref{tools:NtoR}{}, \wref{tools:NRfrom}{},}
...
\item[Translation rules]
	...
	\begin{tabular}{llll}
		...
		\verb|\n|	& newline& \multicolumn{2}{l}{(implementation dependent newline, eg, CR/LF, LF, or LF/CR)} \\
...
\item \stack{}{c-addr}\strike{2.5}{65}

	\sout{\param{c-addr} is the address of the terminal input buffer.}

\note\sout{%
	This word is obsolescent and is included as a concession to
	existing implementations.}

\see \sout{\rref{core:TIB}{}.}

	\begin{rationale} % A.6.2.2290 TIB
		\sout{The function of \word{TIB} has been superseded by
		\word{SOURCE}.}
	\end{rationale}
\end{worddef*}
