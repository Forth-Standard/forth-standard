...
\execute
	...
	\begin{testing} % T.6.1.0080 (
		...
		\test{\word{p} A comment)1234}{\place{x:contribution-132}{1234}} \\
...
\vspace*{-4ex}\enlargethispage{8ex}
...
\vspace*{-1ex}\enlargethispage{3ex}
...
\vspace*{-1.5ex}\enlargethispage{4ex}
\begin{worddef}{0450}{:}[colon]
\item \stack[C]{"<spaces>name"}{colon-sys}

	Skip leading space delimiters. Parse \param{name} delimited by a
	space. Create a definition for \param{name}\remove{x:revise-colon}{,
	called a ``colon definition''}. Enter compilation state and start the current
...
\execute[name]
	...
	\begin{rationale} % A.6.1.0450 :
		% define "colon definition" for uses in the rest of
		% the document
		\place{x:revise-colon-ed}{Words defined with \word{:}
                  are called colon definitions.}
...
\vspace*{-1ex}\enlargethispage{6ex}
...
\vspace*{-1ex}\enlargethispage{4ex}
...
\vspace*{-2.5ex}\enlargethispage{10ex}
...
\item \stack{c-addr_1}{c-addr_2}
	...
	\begin{testing} % T.6.1.0897 CHAR+
		\test{0 \word{CHAR+} 1 \word{=}}{<TRUE>}

		See \tref{core:C,}{C,} for additional test.
	\end{testing}
\end{worddef}
...
\vspace*{-1ex}\enlargethispage{10ex}
...
\item[Quotation]
	The \word{DOES} part inside a quotation
...
\item \stack{c-addr u}{false | i*x true}
	...
	\begin{rationale} % A.6.1.1345 ENVIRONMENT?
		...
		definitions.
\replace{ed21}{The Core word set lacks both a direct method for
		collecting a string in interpretation state (\wref{core:Sq}{S"}
		is in an optional word set) and also a means to test the
		returned flag in interpretation state (e.g. the optional
		\wref{tools:[IF]}{[IF]}).}{The Core word set lack a means to test the
		 returned flag in interpretation state (e.g. the optional
		\wref{tools:[IF]}{[IF]}).}
	\end{rationale}
...
\vspace*{-1ex}\enlargethispage{8ex}
...
\item \stack{c-addr}{c-addr 0 | xt 1 | xt -1}
	...
	\xalso{usage:find}
	\walso[+][x:rule-of-find]{tools:[DEFINED]}
	\walso[+][x:rule-of-find]{search:FIND}
	\ralso{core:'}
	\ralso{core:POSTPONE}
	\ralso{core:[']}
...
\execute
	...
	\begin{rationale} % A.6.1.1760 LEAVE
		...
		\tab \word{:} \texttt{X} {\ldots} \word{DO}
			{\ldots} \word{IF}
				{\ldots} \word{LEAVE}
			\word{THEN} {\ldots} \word{LOOP} {\ldots}
		\word{;}
	\end{rationale}
...
\vspace*{-1.5ex}\enlargethispage{8ex}
...
\item \stack{n_1}{n_2}
	...
	\begin{implement} % I.6.1.1910 NEGATE
		\word{:} \word{NEGATE} \word{p} n1 -{}- n2 ) \\
		\tab \word{INVERT} \word{1+} \\
		\word{;}
	\end{implement}
...
\item \stack{}{}
	...
	\begin{implement} % I.6.1.2050 QUIT
		\word{:} \word{QUIT} \\
...
\compile
	...
	the \word{RECURSE}, not the containing colon definition.
...
\interpret
	...
	Parse \param{ccc} delimited by \texttt{"} (double quote).
	Store the resulting string in a transient buffer \param{c-addr u}.
...
\runtime
	...
	\begin{rationale} % A.6.1.2165 S"
		Typical use: \\
			\tab {\ldots} \word{Sq} \emph{ccc}\texttt{"} {\ldots} \par
			or \par
			\tab \word{:} \texttt{X} {\ldots}
		...
		next execution of \word{Sq} or \word{Seq} in interpretation
		state. It is intended that no standard words other than
		\word{Sq} or \word{Seq} should in themselves cause the
		interpreted string to be overwritten.
		However, since words such as \word[core]{EVALUATE},
		\word[block]{LOAD}, \word[file]{INCLUDE-FILE} and
		\word[file]{INCLUDED} can result in the interpretation of arbitrary
		text, possibly including instances of \word{Sq} or \word{Seq},
	...
	\begin{testing} % T.6.1.2165 S"
		...
		\test{\word{Sq} A String"\word{2DROP}}{} \quad \word{bs} \textdf{There is no space between the " and \word{2DROP}}
	\end{testing}
\end{worddef}
...
\item \stack{d_1 n_1}{n_2 n_3}
	...
	\begin{testing} % T.6.1.2214 SM/REM
		...
		\test{      7 \word{StoD}             -3 \word{SM/REM}}{ 1      -2} \\
		\test{     -7 \word{StoD}              3 \word{SM/REM}}{\replace{x:contribution-138}{1}{-1}      -2} \\
		\test{     -7 \word{StoD}             -3 \word{SM/REM}}{-1       2} \\
...
\vspace*{-2ex}\enlargethispage{8ex}
...
% Output core-ext rationale to a separate file
...
\vspace*{-2.5ex}\enlargethispage{8ex}
...
\execute[xt]
	...
	\note \word{RECURSE} and \word{DOES} are allowed within a \word{:NONAME} definition.
	\end{rationale}

	\begin{testing} % T.6.2.0455 :NONAME
	\word{VARIABLE} nn1 \\
	\word{VARIABLE} nn2 \\
...
\Writetofile{implementation}{\pagebreak}
	\begin{implement} % I.6.2.0698 ACTION-OF
		\word{:} \word{ACTION-OF} \\
	...
	\begin{testing} % T.6.20698 ACTION-OF
...
\execute[name]
	...
	\begin{implement} % I.6.2.0825 BUFFER:
		\dffamily
	...
	\begin{testing} % T.6.2.0825 BU*FFER:
		\ttfamily
		\word{DECIMAL} \\
...
\vspace*{-.5ex}\enlargethispage{8ex}
...
\execute[name]
	...
	\walso{core:ACTION-OF}
	\walso{core:DEFER!}
	\walso{core:DEFER@}
	\walso{core:IS}

	\begin{implement} % I.6.2.1173 DEFER
	...
	\begin{testing} % T 6.2.1173 DEFER
...
\item \stack{xt_2 xt_1}{}
	...
	\begin{implement} % I.6.2.1175 DEFER!
		\word{:} \word{DEFER!} \word{p} xt2 xt1 -{}- ) \\
		\tab \word{toBODY} \word{!} \word{;}
	\end{implement}

	\begin{testing} % T.6.2.1175 DEFER!
...
\item \stack{xt_1}{xt_2}
	...
	\begin{implement} % I.6.2.1177 DEFER@
		\word{:} \word{DEFER@} \word{p} xt1 -{}- xt2 ) \\
		\tab \word{toBODY} \word{@} \word{;}
	\end{implement}

	\begin{testing} % T.6.2.1177 DEFER@
...
\item \stack{c-addr u}{}
	...
	\begin{implement} % I.6.2.1675 HOLDS
	...
	\begin{testing} % T.6.2.1675 HOLDS
...
\item \stack{"<spaces>name<space>"}{c-addr u}
	...
	\begin{implement} % I.6.2.2020 PARSE-NAME
	...
	\begin{testing} % T.6.2.2020 PARSE-NAME
...
% -------------------------------------------------------------------
\vspace*{-1ex}\enlargethispage{12ex}
...
\cbstart\patch{ed21}
	\begin{tabular}{llllllll}
		\verb|\a|	& BEL	& alert,		& ASCII 7 		& \verb|\q|& \multicolumn{2}{l}{double-quote} & ASCII 34\\
		\verb|\b|	& BS		& backspace,	& ASCII 8 		& \verb|\r| & CR	& ASCII 13 \\
		\verb|\e|	& ESC	& escape,		& ASCII 27	& \verb|\t| & HT 	& horizontal tab	& ASCII 9 \\
		\verb|\f|	& FF		& form feed,	& ASCII 12	& \verb|\v| & VT 	& vertical tab 		& ASCII 11 \\
		\verb|\l|	& LF		& line feed,	& ASCII 10	& \verb|\z| & NUL 	& no character 		& ASCII 0 \\
		\verb|\m|	& CR/LF	& pair		& ASCII 13, 10 	& \verb|\"| & \multicolumn{2}{l}{double-quote} & ASCII 34 \\
		\verb|\n|	& newline& \multicolumn{6}{l}{implementation dependent , e.g., CR/LF, CR, LF, LF/CR} \\
...
\item \stack{n_1|u_1 n_2|u_2 n_3|u_3}{flag}
	...
	\begin{rationale} % A.6.2.2440 WITHIN
		...
		\begin{quote}\ttfamily
			\word{:} \word{WITHIN} \word{p} test low high -{}- flag ) \\
			\tab ~\word{OVER} \word{-} \word{toR} \word{-} \word{Rfrom} \word{Uless} \\
			\word{;}
		\end{quote}
	\end{rationale}

	\begin{implement} % I.6.2.2440 WITHIN
		\word{:} \word{WITHIN} \word{p} test low high -{}- flag ) \\
		\tab \word{OVER} \word{-} \word{toR} \word{-} \word{Rfrom} \word{Uless} \\
		\word{;}
	\end{implement}
