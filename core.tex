...
%		\end{quote} 
%
%		However, many systems require a separation of code and data.  Such
%		systems can implement \word{BUFFER:} so that it allocates memory
%		from a separate uninitialized memory area.  Embedded systems can
%		take advantage of the lack of initialization of the memory area
%		while hosted systems are permitted to \word[memory]{ALLOCATE} a
%		buffer at startup.
%		A system may select a piece of memory for performance reasons.
