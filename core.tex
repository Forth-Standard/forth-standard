...
\item[Translation rules]
	...
	parsing and substituting one or more characters as
	\replace{ed21}{follows}{given in table \ref{table:Seq}}, where
	the character after the backslash is case sensitive.

	An ambiguous condition exists if a \bs{} is placed before any character,
	other than those defined \replace{ed21}{in here}{in the table}.

	\begin{table}[ht]
		\caption{\word{Seq} escape sequences}
		\label{table:Seq}
		\begin{center}
\cbstart
			\begin{tabular}[ht]{llll}
				...
				\verb|\n|	& newline& implementation dependent \\
							&			& \multicolumn{2}{l}{(e.g., CR/LF, CR, LF, LF/CR)} \\
				\verb|\q|	&			& double-quote			& ASCII 34 \\
				...
				\multicolumn{3}{l}{\texttt{\bs{}x}\arg{hexdigit}\arg{hexdigit}} \\
					&\multicolumn{3}{p{21em}}{
						The resulting character is the conversion of these two
						hexadecimal digits. An ambiguous conditions exists if
						\texttt{\bs{x}} is not followed by two hexadecimal characters.} \\
				\verb|\z|	& NUL 		& no character 			& ASCII 0	 \\
				\verb|\"|	& 			& double-quote			& ASCII 34 \\
				\verb|\\|	&			& backslash itself		& ASCII 92 \\
				\hline\hline
			\end{tabular}
\cbend
		\end{center}
