...
\see \xref[3.4.1 Parsing]{usage:parsing},
	...
	\begin{rationale} % A.6.1.2410 VARIABLE
		Typical use:
			\remove{ed12}{\ldots} \word{VARIABLE} \texttt{XYZ} \remove{ed12}{\ldots}
	\end{rationale}
...
\cbstart\patch{ed12}
		...
		\tab \uline{\word{DEFER} \texttt{print}} \\[2ex]
		\tab \uline{\word{:NONAME} \word{p} n -{}- ) \word{d} \word{;} \word{IS} \texttt{print}}

	\note \uline{\word{RECURSE} and \word{DOES} shall work within
...
\sout{%
		...
		\begin{quote}\ttfamily
			\sout{\word{:NONAME} {\ldots} code for command 0 {\ldots} \word{;}
			0 CMD \word{!}}

			\sout{\word{:NONAME} {\ldots} code for command 1 {\ldots} \word{;}
			1 CMD \word{!}}

			\tab \sout{\ldots}

			\sout{\word{:NONAME} {\ldots} code for command 99 {\ldots} \word{;}
			99 CMD \word{!}}

			\sout{{\ldots} 5 CMD \word{@} \word{EXECUTE} {\ldots}}
		\end{quote}

\sout{		The definitions of the table building words are:}

		\begin{quote}\ttfamily
			\sout{\word{CREATE} CMD-TABLE
				\word{bs} table for command execution tokens} \\
			\tab \sout{100 \word{CELLS} \word{ALLOT}}

			\sout{\word{:} CMD \word{p} n -{}- a-addr )
				\word{bs} nth element address in table} \\
			\tab \sout{\word{CELLS} CMD-TABLE \word{+}
			\word{;}}
		\end{quote}
...
\sout{%
		...
		\begin{quote}\ttfamily
		  \begin{tabbing}
			...
			\sout{\word{:} DOCOLON \word{p} -{}- )} \\
			\+ \sout{\word{bs} Modify CREATEd word to execute like a colon def} \\
				\+ \sout{\word{DOES} \word{p} i*x a-addr -{}- j*x )} \\
					\sout{1 \word{OVER} \word{+!}}								\> \sout{\word{bs} count executions} \\
			\-	\-	\sout{\word{CELL+} \word{@} \word{EXECUTE}}				\> \sout{\word{bs} execute :NONAME definition} \\
			\sout{\word{;}} \\[1.5\parskip]

			\sout{\word{:} OLD: \word{:} \word{;}}								\>\>\> \sout{\word{bs} just an alias} \\[1.5\parskip]

			\sout{OLD: \word{:} \word{p} "name" -{}- a-addr xt colon-sys )} \\
			\+ \sout{\word{bs} begins an execution-counting colon definition} \\
				\sout{\word{CREATE} \word{HERE} 0 \word{,}}					\>\> \sout{\word{bs} storage for execution counter} \\
				\sout{0 \word{,}}												\>\> \sout{\word{bs} storage for execution token} \\
				\sout{DOCOLON}													\>\> \sout{\word{bs} set run time for CREATEd word} \\
			\-	\sout{\word{:NONAME}}											\>\> \sout{\word{bs} begin unnamed colon definition} \\
			\sout{\word{;}}
		  \end{tabbing}
		\end{quote}
...
\sout{%
		...
		\begin{quote}\ttfamily
		  \begin{tabbing}
			\tab \= \tab \= \hspace*{12em} \= \kill
			\sout{OLD: \word{;} \word{p} a-addr xt colon-sys -{}- )} \\
			\+ \sout{\word{bs} ends an execution-counting colon definition} \\
				\sout{\word{POSTPONE} \word{;}}										\>\> \sout{\word{bs} complete compilation of colon def} \\
			\-	\sout{\word{SWAP} \word{CELL+} \word{!}}								\>\> \sout{\word{bs} save execution token} \\
...
\sout{%
		...
		\tab \sout{{\ldots} \word{'} \texttt{xxx} \word{toBODY} \word[tools]{q} {\ldots}}
\cbend
	\end{rationale}
...
\cbstart\patch{ed12}
		\tab \uline{\word{qDO} {\ldots} \word{LOOP}}

		\tab \sout{\word{:} \texttt{FACTORIAL} \word{p} +n1 -{}- +n2 )~
...
\sout{%
		...
		``}\param{\sout{c-addr u}}\sout{'' stack representation with the alternate
		word \word{Sq}. This may be accomplished by converting
		application words with counted string input arguments to use
		the preferred ``}\param{\sout{c-addr u}}\sout{'' representation, thus
...
\uline{%
	...
	\begin{quote}\ttfamily
	\uline{\ldots} 									\tab[22]		\uline{\word{p} c-addr )} \\
	\uline{\word{FIND} \word{qDUP} \word{IF}}		\tab[17.2]	\uline{\word{p} xt +-1 )} \\
	\tab \uline{\word{STATE} \word{@} \word{IF}}		\tab[17.4]	\uline{\word{p} xt +-1 )} \\
		\tab[2] \uline{\word{0more} \word{IF} \ 
			\word{EXECUTE} \ 
		\word{ELSE} \ 
			\word{COMPILE,} \ 
		\word{THEN}}								\tab[0.2]		\uline{\word{p} ??? )} \\
	\tab \uline{\word{ELSE}}							\tab[21]		\uline{\word{p} xt +-1 )} \\
		\tab[2] \uline{\word{DROP} \word{EXECUTE}} 	\tab[14.6]	\uline{\word{p} ??? )} \\
	\tab \uline{\word{THEN}} \\
		\uline{\word{ELSE}} 							\tab[22]		\uline{\word{p} c-addr )} \\
...
\sout{%
		...
		possible problems:}
		\begin{itemize}
		\item \sout{\word{EVALUATE} is slower than \word{COMPILE,} because a
...
\item \stack{xt_2 xt_1}{}
	...
	condition exists if \param{xt_1} is not for a word defined
	\replace{ed12}{via}{ by}
	\word{DEFER}.
...
\item \stack{xt_1}{xt_2}
	...
	\replace{ed12}{with}{ by} \word{DEFER},
...
\sout{%
			...
			\begin{quote}
				\sout{\texttt{PARSE-WORD}} \qquad
					\stack{\sout{"<spaces>name"}}{\sout{c-addr u}} \\
...
\uline{%
			The behavior of \word{WORD} with respect to skipping leading
			delimiters is useful for parsing blank delimited names.
			\word{PARSE-NAME} provides this service in a similar manner to
			\word{PARSE} with respect to the ``}\param{\uline{c-addr u}}\uline{'' return value,
...
\place{ed12}{\wref{double:2VALUE}{2VALUE},
	...
	\remove{ed12}{%
		...
		We explicitly require that \word{TO} must parse, so that
	...
	\place{ed12}{%
		...
		a mode flag that is tested by the\linebreak subsequent execution of
