...
\subsection{Environmental queries\strike{2.4}{155}} % 7.3.1
\cbstart\patch{x:wordset-query}%

\sout{Append table \ref{block:env} to table \ref{table:env}.}
...
\begin{table}[h]
  \begin{center}
	...
	\begin{tabular}{p{9em}rcp{0.42\textwidth}}
		...
		\hline
		\sout{\texttt{BLOCK}}		& \sout{\emph{flag}}	& \sout{no}	&
			\sout{block word set present} \\
		\sout{\texttt{BLOCK-EXT}}	& \sout{\emph{flag}}	& \sout{no}	&
			\sout{block extensions word set present} \\
		\hline\hline
	\end{tabular}
  \end{center}
\end{table}
