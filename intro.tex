...
\replace{x:legacy}{because }{as}
they may be withdrawn from future revisions of the Standard.

This Standard designates the following word\remove{x:legacy}{s} as obsolescent:

\setwordlist{core}
\cbstart
\begin{tabular}{rl@{\qquad}rl@{\qquad}rl}
  \sout{\ref{core:numTIB}}		& \sout{\word{numTIB}}
& \ref{tools:FORGET}	& \word[tools]{FORGET}
& \sout{\ref{core:SPAN}}		& \sout{\word{SPAN}} \\
  \sout{\ref{core:CONVERT}}	& \sout{\word{CONVERT}}
& \sout{\ref{core:QUERY}}		& \sout{\word{QUERY}}
& \sout{\ref{core:TIB}}			& \sout{\word{TIB}} \\
  \sout{\ref{core:EXPECT}}		& \sout{\word{EXPECT}} \\
\end{tabular}
\cbend
...
\item \place{x:wordset-query}{%
	Using \word{ENVIRONMENTq} to enquire whether a word set is present.
}
\end{itemize}
