...
%		\textsf{ed09}:
%			Changed ``associated with'' to ``assigned to''
%			or ``assigned an'' in the definition of
%			\wref{local:LOCAL}{}.
%
...
\version{16.1}{Bath Meeting}{30 September -- 2 October, 2015}
	\begin{itemize}
	\item \xref{members}:						% members-2x
		\begin{enumerate}
	...
	\item[3] Usage requirements:					% usage
	\begin{enumerate}
		\item \textsf{x:2-complement}: Removed ``Programs that use flags as arithmetic operands have an environmental dependency.'' from \xref{usage:flags}.
		\item \textsf{x:2-complement}: Replaced -32767 with -32768 in \xref{usage:int}.
		\item \textsf{x:2-complement}: Removed reference to alternate arithmetic  architectures from \xref{usage:number}.
		\item \textsf{x:2-complement}: Replaced implementation defined response to overflow/underflow of signed numbers with specific text in \xref{usage:intops}.
	\end{enumerate}
	...
	\item[12] Floating-Point Word Set:				% float
		\begin{enumerate}
		\item \textsf{ed16}: Replaced \texttt{0E} with \texttt{-0E} in  \wref{floating:FATAN2}{}.
		\end{enumerate}

	\item[A] Rationale (Annex \ref{annex:rationale}):				% rationale
		\begin{enumerate}
		\item \textsf{x:2-complement}: Removed number representations from \xref{rat:char}.
		\item \textsf{ed16}: Removed reference to EBCDIC from \xref{rat:char}.
		\item \textsf{x:2-complement}: Remove discussion of alternative arithmetic architectures from \xref{rat:env:num}.
		\item \textsf{x:2-complement}: Revised example in A.3.2.2.2 Other integer operations.
		\item \textsf{x:2-complement}: Replaced environmental dependency example in  \xref{rat:label:prog}.

		% Core
		\item \textsf{x:2-complement}: Removed reference
			to two's-complement from \rref{core:WITHIN}{}.

		% Double
		\item \textsf{x:2-complement}:
			Replaced rational for \rref{double:DtoS}{}.
		\end{enumerate}
	...
	\item[E] Reference Implementations (Annex \ref{annex:implement}):	% imp
		...
		% Core
		\item \textsf{x:2-complement}: Added \iref{core:WITHIN}{}.

		% Double
		\item \textsf{x:2-complement}: Added \iref{double:DtoS}{}.
		\end{enumerate}

	\item[F] Test Suite (Annex \ref{annex:test}):						% test
		% Core
		\begin{enumerate}
		\item \textsf{x:2-complement}: Added test case to \tref{core:1+}{}.
		\item \textsf{x:2-complement}: Added test case to \tref{core:INVERT}{}.
		\end{enumerate}
	\end{itemize}

% =========================================================

\version{17.1}{Konstanz Meeting}{7--9 September, 2016}
	\begin{itemize}
	\item Title page:
		\begin{enumerate}
	...
	\item[3] Usage requirements:					% usage
		\begin{enumerate}
		\item \textsf{x:1 chars = 1}: Replace ``at least one'' with ``exactly one'' in \xref{usage:char};
		\item \textsf{x:2-complement}: Replaced ``two's complement'' with ``two's-complement'' in \xref{usage:number};
		\item \textsf{x:2-complement}: Replaced ``divisions'' with ``division'' in \xref{usage:intops};
		\item \textsf{x:2-complement}: Removed ``in these case'' from \xref{usage:intops};
		\item \textsf{x:2-complement}: Added ``for operations other than division'' to \xref{usage:intops};
		\item \textsf{x:2-complement}: Replaced ``$2^{n-1}$'' with ``$-2^{n-1}$'' in \xref{usage:intops};
		\item \textsf{x:reset}: Removed ``X:wordset-query'' from \xref{usage:obsolete};
	...
	\item[12] Floating-Point Word Set:			% float
		\begin{enumerate}
		\item \textsf{x:to-f-round}: Removed \word[floating]{DtoF} and \word[floating]{StoF} conditions from \xref{float:ambiguous};
		\item \textsf{x:to-f-round}: Revised definition of \wref{floating:DtoF}{};
		\item \textsf{x:to-f-round}: Revised definition of \wref{floating:StoF}{}.
		\end{enumerate}

	\item[A] Rationale (Annex \ref{annex:rationale}): 	% rationale
		\begin{enumerate}
		\item \textsf{x:2-complement}: Made definition of $n$ a clause of previous sentence in \xref{rat:env:num};
		\item \textsf{x:2-complement}: Removed ``likely'' from second paragraph of \xref{rat:env:num};
		\item \textsf{x:2-complement}: Removed final paragraph of \xref{rat:env:num};
		\item \textsf{x:2-complement}: Remove A.3.2.2.2 Other integer operations;
		\item \textsf{x:2-complement}: Replaced ``entitle'' with ``entitled'' in \xref{rat:label:prog};
		\item \textsf{x:2-complement}: Replaced ``lower'' with ``lowest'' in \xref{rat:label:prog};

		% Double
		\item \textsf{x:2-complement}: Replaced ``twos-complement'' with ``two's complement'' in \rref{double:DtoS}{}.
		\end{enumerate}

	\item[E] Reference Implementations (Annex \ref{annex:implement}):	% imp
		\begin{enumerate}
		% Core
		\item \textsf{x:2-complement}: Added \iref{core:NEGATE}{};

		% Double
		\item \textsf{x:2-complement}: Corrected ``S>D'' to ``D>S'' in \iref{double:DtoS}{}.
		\end{enumerate}

	\item[F] Test Suite (Annex \ref{annex:test}):	% test
		\begin{enumerate}
		% Core
		\item \textsf{x:1 chars = 1}: Added test case to \tref{core:CHAR+}{};
		\item \textsf{x:1 chars = 1}: Added test case to \tref{core:CHARS}{}.
		\end{enumerate}
...
\version{18.1}{Bad V\"oslau Meeting}{6--8 September, 2017}
	\begin{itemize}
	\item \xref{foreword}:						% foreword 
	\begin{enumerate}
	...
	\item[3] Usage requirements:				% usage
	\begin{enumerate}
		\item \textsf{ed18}: The Exception word set is no longer optional (\xref{usage});
		\item \textsf{ed18}: Moved \ref{annex:rationale}.11.3.4 Other transient regions to \xref{usage:"literal};
	...
	\item[4] Documentation requirements:	% doc
	\begin{enumerate}
		\item \textsf{ed18}: Moved \xref{file:impopt} documentation requirements to \xref{doc:imp}.
	\end{enumerate}
	
	\item[6] Core Word Set:					% core
	\begin{enumerate}
		\item \textsf{x:quotation}: Added quotation ambiguous condition to \wref{core:;}{};
		\item \textsf{x:quotation}: Added quotation ambiguous condition to \wref{core:DOES}{};
		\item \textsf{x:quotation}: Added quotation requirement to \wref{core:RECURSE}{};
		\item \textsf{ed18}: Merged 11.6.1.2165 \texttt{S"} with \wref{core:Sq}{};
		\item \textsf{ed18} Added cross reference to \wref{core:Cq}{};
		\item \textsf{ed18}: Merged 11.6.2.2266 \texttt{S\bs{"}} with \wref{core:Seq}{}.
	\end{enumerate}
	
	\item[9] Exception Word Set:				% exception
	\begin{enumerate}
		\item \textsf{ed18}: The Exception word set is not longer optional (\xref{wordlist:exception});
		\item \textsf{ed18}: Removed 9.5 Compliance and labeling.
	\end{enumerate}

	\item[11] File-Access Word Set:			% file
	\begin{enumerate}
		\item \textsf{ed18}: Merged \xref{file:buffers} with \xref{usage:"literal};
		\item \textsf{ed18}: Removed \xref{file:buffers} related system documentation
			 requirement;
		\item \textsf{ed18}: Removed 11.6.1.2165 \texttt{S"}.
		\item \textsf{ed18}: Removed 11.6.2.2266 \texttt{S\bs{"}}.
	\end{enumerate}

	\item[15] Programming-Tools Word Set:	% tools
	\begin{enumerate}
		\item \textsf{x:quotations}: Added \xref{tools:quotation-sys};
		\item \textsf{x:quotations}: Extended \xref{tools:impopt};
		\item \textsf{x:quotations}: Added ambiguous condition to \wref{tools:;CODE}{};
	...
	\item[A] Rationale (Annex \ref{annex:rationale}):	% rationale
	\begin{enumerate}
		\item \textsf{ed18}: Added \xref{rat:"literal};
% Core
		\item \textsf{ed18}: Merged \ref{annex:rationale}.11.6.1.2165 \texttt{S"} with \rref{core:Sq}{};
% File
...
% Core
	...
	\begin{enumerate}
% Core
		\item \textsf{ed18}: Moved \ref{annex:test}.11.6.1.2165 \texttt{S"} to \tref{core:Sq}{};
% File
...
\version{19.1}{Queensferry Meeting}{12--14 September 2018}
\begin{itemize}
	\item \xref{foreword}:						% foreword 
	\begin{enumerate}
	...
	\item[15] Programming-Tools Word Set:	% tools
	\begin{enumerate}
		\item \textsf{x:find-name:} Added \wref{tools:FIND-NAME}{} and \wref{tools:FIND-NAME-IN}{};
	\end{enumerate}

	\item[A] Rationale (Annex \ref{annex:rationale}):	% rationale
	\begin{enumerate}
		\item \textsf{x:find-name:} Added \rref{tools:FIND-NAME}{};
	\end{enumerate}

	\item[E] Reference Implementations (Annex \ref{annex:implement}): % imp
	\begin{enumerate}
		\item \textsf{x:find-name:} Added \iref{tools:FIND-NAME}{} and \iref{tools:FIND-NAME-IN}{}.
	\end{enumerate}

	\item[F] Test Suite (Annex \ref{annex:test}):	% test
	\begin{enumerate}
		\item \textsf{x:find-name:} Added \tref{tools:FIND-NAME}{} and \iref{tools:FIND-NAME-IN}{}.
	\end{enumerate}
\end{itemize}

% =========================================================

\version{20.1}{Hamburg Meeting}{{11--13} September 2019}
	\begin{itemize}
	\item \xref{foreword}:					% foreword 
	\begin{enumerate}
	...
	\begin{enumerate}
	\item \textsf{x:defined-find}: Added reference to \xref{search:terms} to ``find''.
	\end{enumerate}

	\item[15] Programming-Tools Word Set:	% tools
	\begin{enumerate}
	\item \textsf{x:defined-find}: Removed reference to \word{FIND} from \wref{tools:[DEFINED]}{};
	\item \textsf{x:name-token}: Added name token to \xref{tools:terms}.
	\end{enumerate}

	\item[16] Search-Order Word Set:		% search
	\begin{enumerate}
	\item \textsf{x:defined-find}: Added ``find'' to \xref{search:terms}.
	\end{enumerate}
...
% Tools
	...
	\item \textsf{x:else-then}: Add \word[tools]{[ELSE]} without proceeding \word[tools]{[IF]} is not
		recommended to \rref{tools:[ELSE]}{};
	\item \textsf{x:name-token}: Removed A.15.3.1 Name tokens.
	\end{enumerate}
\end{itemize}

% =========================================================

\version{21.1}{Online}{{1--3} September 2020}
	\begin{itemize}
	\item \xref{foreword}:						% foreword 
		\begin{enumerate}
	...
	\item[6] Core Word Set:					% core
		...
		\item \textsf{revise-colon}: Remove ", called a "colon definition" from
			first paragraph of \wref{core::}{} (colon);
		\item \textsf{x:rules-of-find}: Added cross references for
			\wref{tools:[DEFINED]}{} and \wref{search:FIND}{}
			to \wref{core:FIND}{}.
		\item \textsf{ed21}: Moved escape sequences in \wref{core:Seq}{} to a table
			figure with minor reformatting of the table.
		\end{enumerate}
	\item[13] Locals Word Set:				% local
		\begin{enumerate}
		\item Add \emph{local} interpretation semantics to \wref{local:LOCAL}{};
		\item Remove ambiguous condition from \emph{name} execution semantics of \wref{local:b:}{};
		\item Add \emph{name} interpretation semantics to \wref{local:b:}{};
		\end{enumerate}
	\item[15] Programming-Tools Word Set:	% tools
		\begin{enumerate}
		\item \textsf{x:cs-drop}: Added \wref{tools:CS-DROP}{};
		\item \textsf{x:cs-drop}: Changed stack description of \wref{tools:CS-PICK}{};
		\item \textsf{x:traverse-wordlist}: Wording changes and new paragraph added
			to \xref{tools:TRAVERSE-WORDLIST};
		\item \textsf{x:[if]-input}: Revised input of \wref{tools:[IF]}{} to match
			\wref{core:IF}{};
		...
		\item \textsf{ed21}: Add reference to \word[tools]{[IF]} and \word[tools]{[ELSE]}
			to \wref{tools:[THEN]}{};
		\item \textsf{x:simplify-bracket-else}: Add reference to the implementation of
			\word[tools]{[ELSE]} to:
			\wref{tools:[IF]}{},
			\wref{tools:[ELSE]}{} and
			\wref{tools:[THEN]}{};
		\item \textsf{x:simplify-bracket-else}: Add reference to the testing of
			\word[tools]{[THEN]} to:
			\wref{tools:[IF]}{},
			\wref{tools:[ELSE]}{} and
			\wref{tools:[THEN]}{};
	...
	\item[16] Search-Order Word Set:		% search
		...
		\item \textsf{x:vote-9}: Remove ambiguous condition from
			\xref{search:find};
		\item \textsf{x:rules-of-find}: Add cross reference for
			\wref{tools:[DEFINED]}{} to \wref{search:FIND}{};
		\item \textsf{x:vocabulary}: Added \wref{search:VOCABULARY}{};
		\end{enumerate}
	\item[17] String Word Set:				% string
		\begin{enumerate}
		\item \textsf{x:revise-replaces}: Revise last paragraph of \wref{string:REPLACES}{}.
		\end{enumerate}
	\item[A] Rationale (Annex \ref{annex:rationale}):	% rationale
		\begin{enumerate}
% Core
		\item \textsf{ed21}: Remove reference to interpretation version of S" from \rref{core:ENVIRONMENTq};
% Block
% Double
% Exception
% Facility
...
% Memory
% Tools
		\item \textsf{x:cs-drop}: Added \rref{tools:CS-DROP}{};
		\item \textsf{x:rule-of-find}: Added cross references for
			\wref{core:FIND}{} and \wref{search:FIND}{} to
			\wref{tools:[DEFINED]}{};
% Search
		\item \textsf{x:vocabulary}: Added \rref{search:VOCABULARY}{};
% String
% Xchar
		\end{enumerate}
	\item[E] Reference Implementations (Annex \ref{annex:implement}): % imp
		\begin{enumerate}
		\item \textsf{ref-license}: Add a license to use the reference
			implementations to \xref{ref:intro}.
% Core
% Block
% Double
% Exception
...
% Memory
% Tools
		\item \textsf{x:cs-drop}: Added \iref{tools:CS-DROP}{};
		\item \textsf{x:synonym} Remove \iref{tools:SYNONYM}{}; % E.15.6.2.2264 
		\item \textsf{simplify-bracket-else}: Replace reference implementation for
			\iref{tools:[ELSE]}{} and remove reference implementations
			for \iref{tools:[IF]}{} and \iref{tools:[THEN]}{};
% Search
		\item \textsf{x:vocabulary} Added \iref{search:VOCABULARY}{};
% String
% Xchar
		\end{enumerate}
	\item[F] Test Suite (Annex \ref{annex:test}):	% test
		\begin{enumerate}
		\item \textsf{ed21}: Fix typo in final paragraph of \xref{test:intro}.
% Core
% Block
% Double
% Exception
...
% Memory
% Tools
		\item \textsf{x:cs-drop}: Added \tref{tools:CS-DROP}{};
% Search
% String
% Xchar
		\end{enumerate}
