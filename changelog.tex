...
\item[07.2]
	...
	\item \xref{process}:
		Two new sentances (\textsf{ed07}).

	\item \xref{members}:
		Membership Rules (\textsf{ed07}).

	\item[3] Usage Requirements:
		Added \textsf{X:strucutres} and \textsf{X:throw-iors}
		to table \xref{table:extensions}.
	...
	\item[B] Bibliography (Annex \ref{annex:bib}):
		\begin{enumerate}
		\item \textsf{ed07}: Added books by Elizabeth Rather and Stephen Pelc.
