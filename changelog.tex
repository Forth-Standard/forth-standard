...
\item[07.1]
	...
	\item Added new ``\xref{foreward}'' and ``\xref{process}'' as defined
		in \textsf{X:foreward}.

	\item Replaced ``X3 Membership'' with ``\xref{members}'' as
		defined in \textsf{X:foreward}.
	...
	\item[B] Added \emph{ANS Forth} and \emph{ISO Forth} to
		``Industry standards'' section of \xref{annex:bib}
		(\textsf{X:foreward}).

	\item[F] Added John Hayes' core test suite, changing the
		introduction in the process. (\xref{annex:test})
