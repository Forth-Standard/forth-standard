...
\subsection{Environmental queries\strike{2.4}{160}} % 15.3.1
...
\item \emph{name} can't be found (\wref{tools:FORGET}{}\place{x:synonym}{, \wref{tools:SYNONYM}{}});
...
\item \place{x:synonym}{\wref{core:IMMEDIATE}{} is applied to a word defined by \wref{tools:SYNONYM}{}.}
\item \place{x:n-to-r}{\wref{tools:NRfrom}{} is used with data not stored by
	\wref{tools:NtoR}{}.}
\end{itemize}
...
\sout{%
	...
	\uline{%
		\word{FORGET} tries to infer information about the previous dictionary
		state from the forgotten word, which is not always possible.  In
		consequence, \word{FORGET} is guaranteed to restore the state of name
		space, but not necessarily the state of dictionary.}

	\uline{For example,}
		\begin{quote}
			\uline{\word{ALIGN} \word{HERE} 100 \word{CELLS} \word{ALLOT} \word{CONSTANT} mybuffer} \\
			\uline{\word{FORGET} mybuffer}
		\end{quote}
		\uline{in this case the 100 reserved cells are a part of the buffer,
		but \word{FORGET} can not infer this.  A solution to the restoration
		of the dictionary state is provided by \word{MARKER}.}

	\uline{%
		Nevertheless, \word{FORGET} is a useful tool for interactive sessions,
		because it does not require creation of a marker in advance.  An example
		session may look like following:}

		\begin{quote}
			\uline{\word{:} foo {\ldots} foo {\ldots} \word{p} buggy redefinition) \word{;}} \\
			\uline{finding out the bug} \\
			\uline{\word{FORGET} foo} \\
			\uline{\word{:} foo {\ldots} foo {\ldots} \word{p} bug fixed) \word{;}} \\
...
\see \wref{tools:NRfrom}{},
	...
	\see \uline{\wref{core:SAVE-INPUT}{},
