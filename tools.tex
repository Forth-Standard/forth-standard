...
\see \xref[3.4.1 Parsing]{usage:parsing},
	...
	For example,
		\begin{quote}
			\word{ALIGN} \word{HERE} 100 \word{CELLS} \word{ALLOT} \word{CONSTANT} mybuffer \\
			\word{FORGET} mybuffer
		\end{quote}
		...
		\begin{quote}
			\word{:} foo {\ldots} foo {\ldots} \word{p} buggy redefinition) \word{;} \\
			finding out the bug \\
			\word{FORGET} foo \\
			\word{:} foo {\ldots} foo {\ldots} \word{p} bug fixed) \word{;} \\
