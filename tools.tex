...
\see \rref{tools:CS-PICK}{}.
	...
	\begin{rationale} % A.15.6.2.1015 CS-PICK
		\patch{x:cs-drop}\sout{The intent}\uline{A common way to use \word{CS-PICK}} is to copy
                \param{dest} \patch{x:cs-drop}\sout{on the control-flow stack} so that it can be resolved more than
		...
		\setwordlist{tools}
                ...
                \patch{x:cs-drop}\uline{You can also use \word{CS-PICK} to copy an
...
\see \place{x:synonym}{\rref{tools:SYNONYM}{}}, \wref{core:IMMEDIATE}{}.

	\begin{rationale}
	  \cbstart\patch{x:synonym}%
          The implementation of \word{tools:SYNONYM} requires detailed knowledge of
