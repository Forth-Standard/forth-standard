...
\begin{itemize}
\item \remove{x:legacy}{deleting the compilation word-list (\wref{legacy:FORGET}{FORGET});}
\item fewer than $u+1$ items on control-flow stack
	(\wref{tools:CS-PICK}{CS-PICK},
	 \wref{tools:CS-ROLL}{CS-ROLL});
\item \remove{x:legacy}{\emph{name} can't be found (\wref{legacy:FORGET}{FORGET});}
...
\item reaching the end of the input source before matching
	...
	(\wref{tools:[IF]}{[IF]});
\item \remove{x:legacy}{removing a needed definition (\wref{legacy:FORGET}{FORGET}).}
\end{itemize}
