...
\item \place{x:cs-drop}{resolving an already resolved \param{orig}
  forward reference (\wref{tools:CS-PICK}{CS-PICK}, \wref{core:ELSE}{ELSE}, \wref{core:THEN}{THEN},
  \wref{core:REPEAT}{REPEAT});}
\item \place{x:cs-drop}{at the end of the current definition
  unresolved \param{orig} forward references still exist
  (\wref{tools:CS-DROP}{CS-DROP}, \wref{core:;}{;}, \wref{core:IF}{IF}, \wref{core:ELSE}{ELSE});}
...
\execute
	...
	not a \param{dest} or an \param{orig}, or if the control-flow stack is empty.

	\begin{rationale} % A.15.6.2.---- CS-DROP
		Note that for every \word{CS-DROP} of an \param{orig}, there
		must be a \word{CS-PICK} of that \param{orig}, because every
		\param{orig} must be resolved exactly once (see
		\ref{tools:ambiguous}).
...
\cbend
	...
	\begin{rationale} % A.15.6.2.1015 CS-PICK
		\replace{x:cs-drop}{The intent}{A common way to use \word{CS-PICK}} is to copy
                \param{dest} \remove{x:cs-drop}{on the control-flow stack} so that it can be resolved more than
                once. For example: \setwordlist{core}
		\begin{quote}\ttfamily
		...
		\setwordlist{tools}
                ...
                \place{x:cs-drop}{You can also use \word{CS-PICK} to copy an
                \param{orig}, but then one of the resulting copies
                must be \word{CS-DROP}ped because every \param{orig}
                must be resolved exactly once (see
                \ref{tools:ambiguous}).}
	\end{rationale}
...
\item \stack{c-addr u}{nt|0}
	...
	\walso{tools:FIND-NAME-IN}

	\begin{rationale}
		\word{FIND-NAME} and \word{FIND-NAME-IN} are natural factors of all words
		that look up words in the dictionary, such as \word{FIND}, \word{'},
		\word{POSTPONE}, the text interpreter, and \word[search]{SEARCH-WORDLIST}.
		So implementing them does not cost additional code, only some refactoring
		effort.

		This approach is not compatible with system that use separate word headers
		for	the interpretation and compilation semantics of a word.  This problem
		already exists for the other words that deal with name tokens.  However,
		such systems have been known for a lest two decades, and have seen little
		to no uptake in standard systems.
...
\item \stack{c-addr u wid}{nt|0}
	...
	\xalso{tools:FIND-NAME}
	\ralso{tools:FIND-NAME}
...
\enlargethispage{8ex}\vspace*{-2.5ex}
...
\item[\param{newname} compilation]
	...
	\walso{core:IMMEDIATE}

	\begin{rationale}
	  \cbstart\patch{x:synonym}%
          The implementation of \wref{tools:SYNONYM}{} requires detailed knowledge of
          the host implementation, which is why it is standardized.
          \cbend
        \end{rationale}
...
\place{x:traverse-wordlist}{%
	...
	\walso{tools:NAMEtoSTRING}
	\walso{tools:NAMEtoINTERPRET}
	\walso{tools:NAMEtoCOMPILE}
...
\compile
	...
	\begin{implement}
		\textdf{%
			...
			is an outline definition where \texttt{save-definition-state} and
			\texttt{restore-definition-state} require carnal knowledge of the
			system and are left to the implementor.
		}
		
		\word{:} \word{[:} \word{p} C: -{}- quotation-sys colon-sys ) \\
		\tab \word{POSTPONE} \word{AHEAD} save-definition-state \word{:NONAME} \\
		\word{;} \word{IMMEDIATE}
		
		\word{:} \word{;]} \word{p} C: quotation-sys colon-sys -{}- ) ( -{}- xt ) \\
		\tab \word{POSTPONE} \word{;} \word{toR} restore-definition-state \\
...
\execute
	...
	\xalso[+][x:rule-of-find]{core:FIND}
	\xalso[+][x:rule-of-find]{search:FIND}
...
\execute
	...
	\walso[+]{tools:[IF]}
	\walso[+]{tools:[THEN]}
	\talso[+][x:simplify-bracket-else]{tools:[THEN]}
...
\cbstart\patch{x:simplify-bracket-else}
		\begin{tabbing}
...
\execute
	...
	\walso[+]{tools:[ELSE]}
	\walso[+]{tools:[THEN]}
	\ialso[+][simplify-bracket-else]{tools:[ELSE]}
	\talso[+][simplify-bracket-else]{tools:[THEN]}
...
\execute
	...
	\walso[+]{tools:[IF]}
	\walso[+]{tools:[ELSE]}	
	\ialso[+][x:simplify-bracket-else]{tools:[ELSE]}
