...
\cbstart\patch{x:traverse-wordlist}
\subsection[Data types]{\uline{Data types}} % 15.3.1
...
\uline{Append table \ref{tools:types} to table \ref{table:datatypes}.}
...
\begin{table}[h]
  \begin{center}
	...
	\hline
	\emph{\uline{nt}}	& \uline{name token} & \uline{1 cell} \\
	\hline\hline
	\end{tabular}
  \end{center}
\end{table}
...
\see \uline{\xref{tools:TRAVERSE-WORDLIST}{}.}
\end{worddef*}
...
\uline{See \wref{tools:TRAVERSE-WORDLIST}{}.}
\end{worddef*}
...
\see \uline{\rref{tools:TRAVERSE-WORDLIST}{},
	...
	\begin{rationale}
		...
		\uline{prints a count of the number of words in the \word[search]{FORTH-WORDLIST}.}
...
\uline{%
		...
		\uline{prints the names of words in the current compilation wordlist.}

		\begin{quote}\ttfamily
\uline{%
			\word{:} CONTAINS-STRING} \\
\tab \uline{%
				\word{NAMEtoSTRING} \word{2OVER}
...
\uline{%
		...
		\uline{prints the name of a word containing the string
