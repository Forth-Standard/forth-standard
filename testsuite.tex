...
Applied Physics Laboratory developed a \replace{test}{validation}{test} suite. In
...
``\verb|{|'' \place{test}{(open brace)} followed by a code sequence to
test. This is followed by ``\verb|->|'', the expected results, and
``\verb|}|'' \place{test}{(close brace)}. For example, the following:
\begin{quote}
	\verb|{ 1 1 + -> 2 }|
\end{quote}
...
Forth CORE word set.
\remove{test}{The test starts by verifying basic assumptions about
number representation. It then builds on this with tests of boolean
logic, shifting, and comparisons. It then tests basic stack
manipulations and arithmetic. Ultimately, it tests the Forth interpreter
and compiler.}


\section{Tester Source}
The following source code provides the
\replace{test}{test harness described in section \textbf{??}}{tester}.
...
\place{test}{The test starts by verifying basic assumptions about number
representation. It then builds on this with tests of boolean logic,
shifting, and comparisons. It then tests basic stack manipulations and
arithmetic. Ultimately, it tests the Forth interpreter and compiler.}

\place{test}{The tests presented here are John Hayes' original test
which accompany the tester package. Additional test have been given
in the rationale for individual words.}

\remove{test}{In this and following sections we present validation tests
for specific words. A standard system would be expected to pass \emph{all}
of the validation tests documented here.}

\remove{test}{The words in this section are organized by word set,
retaining their index numbers for easy cross-referencing to the glossary.}
