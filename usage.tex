...
Standard \replace{ed07}{where labelled}{are labeled} as \emph{extensions}. These extensions have
been added to the environmental query name space. \word{ENVIRONMENTq}
\replace{ed07}{a}{returns} \emph{true} if the system has implemented the extension as
...
%	\place{x:fp-stack}{\texttt{X:fp-stack}} & -- & -- & \place{x:fp-stack}{the \textsf{X:fp-stack} extension is present} \\
	\place{x:number-prefix}{\texttt{X:number-prefix}} & -- & -- & \place{x:number-prefix}{the \textsf{X:number-prefix} extension is present} \\
