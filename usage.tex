...
\remove{x:key-ekey}{Characters shall be at least one address unit
wide, contain at least eight bits, and have a size less than or equal
to cell size.}

\place{x:key-ekey}{Characters shall have the following properties:}
\begin{itemize}
\item \place{x:key-ekey}{at least one address unit wide;} \\[-4ex]
\item \place{x:key-ekey}{contain at least eight bits;} \\[-4ex]
\item \place{x:key-ekey}{be of fixed width;} \\[-4ex]
\item \place{x:key-ekey}{have a size less than or equal to cell size;} \\[-4ex]
\item \place{x:key-ekey}{be unsigned.}
\end{itemize}
...
\place{x:namelen}{A system may allow the creation of definition
names longer than 31 characters.  Programs with definition names
longer than 31 characters have an environmental dependancy.}
