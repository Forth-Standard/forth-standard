...
\subsection{Environmental queries\strike{2.4}{155}} % 9.3.4
\cbstart\patch{x:wordset-query}%

\sout{Append table \ref{exception:env} to table \ref{table:env}.}
...
\begin{table}[ht]
  \begin{center}
	...
	\begin{tabular}{p{9em}rcp{0.42\textwidth}}
		...
		\hline
		\sout{\texttt{EXCEPTION}}		& \sout{\emph{flag}}	& \sout{no}	&
			\sout{Exception word set present} \\
		\sout{\texttt{EXCEPTION-EXT}}	& \sout{\emph{flag}}	& \sout{no}	&
			\sout{Exception extensions word set present} \\
		\hline\hline
	\end{tabular}
  \end{center}
\end{table}
...
\throwdef[x:xchar]{-77}{Malformed xchar}
			\end{minipage}
			\\[-2ex] \hline\hline
		\end{tabular}
	\end{center}
