...
\subsection{Environmental queries\strike{2.4}{160}} % 11.3.3
\cbstart\patch{x:wordset-query}%

\sout{Append table \ref{file:env} to table \ref{table:env}.}
...
\begin{table}[ht]
  \begin{center}
	...
	\begin{tabular}{p{9em}rcp{0.42\textwidth}}
		...
		\hline
		\sout{\texttt{FILE}}		& \sout{\emph{flag}}		& \sout{no}	&
			\sout{file word set present} \\
		\sout{\texttt{FILE-EXT}}	& \sout{\emph{flag}}		& \sout{no}	&
			\sout{file extensions word set present} \\
		\hline\hline
	\end{tabular}
  \end{center}
\end{table}
