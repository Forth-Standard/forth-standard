...
\item[\uline{find (update the definition in \xref{notation:terms}):}]
  \place{x:rules-of-find}{To search the search order or a specified
    wordlist for a definition name matching a given string.}
...
\item \place{x:vocabulary}{%
	executing a vocabulary word when the search order is empty
	(\wref{search:VOCABULARY}{}).}
\end{itemize}
...
\see \place{ed21}{}\xref[16.3.3 Finding definition names]{search:find},
      %the above replaced a reference to 3.4.2 Finding definition names;
      %but I could not get \replace or \sout or \uline to work
	\wref{core:'}{'},
	\wref{core:FIND}{FIND},
	\wref{core:POSTPONE}{POSTPONE},
	\wref{core:[']}{[']},
	\place{x:rules-of-find}{\wref{tools:[DEFINED]}{}}.
...
\item \stack{"<spaces>name"}{}
	...
	Skip leading space delimiter. Parse \param{name} delimited by a space.
	Create a definition for \param{name} with the execution semantics defined
	below. Create a new empty word list \param{wid} and associate it with
	\param{name}.

	\param{name} is referred to as a "vocabulary".

\item[\param{name} Execution:]
	\stack{}{}

	Replace the first word list in the search order by the word list \param{wid}
	that is associated with the \param{name} vocabulary. An ambiguous condition
	exists if there are no word lists in the search order.

\see \rref{search:VOCABULARY}{},
	\iref{search:VOCABULARY}{},
	\wref{search:WORDLIST}{},
	\wref{search:ALSO}{},
	\wref{search:PREVIOUS}{},
	\wref{search:GET-ORDER}{},
	\wref{search:SET-ORDER}{}.

	\begin{rationale}
		\word{VOCABULARY} has been used in traditional Forth systems and it is
	...
	\item[Typical use]
		\word{VOCABULARY} \texttt{MYVOC}

		\word{ONLY} \word{FORTH} \word{ALSO} \texttt{MYVOC} \word{DEFINITIONS}

                The last line sets the search order to search
                \texttt{MYVOC} first and \word{FORTH} second, and it
                sets the compilation wordlist to \texttt{MYVOC}.
	\end{rationale}

	\begin{implement}
		\word{:} \word{VOCABULARY} \word{p} -{}- ) \\
		\tab \word{WORDLIST} \word{CREATE} \word{,} \\
		\tab \word{DOES} \word{p} -{}- ) \\
		\tab[2] \word{@} \word{toR} \word{GET-ORDER} \word{SWAP} \word{DROP} \\
		\tab[2] \word{Rfrom} \word{SWAP} \word{SET-ORDER} \\
		\word{;}
	\end{implement}
\end{worddef*}
