...
\begin{table}[ht]
  \begin{center}
	...
	\begin{tabular}{p{10em}rcp{0.42\textwidth}}
		...
		\hline
		\remove{x:wordset-query}{\texttt{SEARCH-ORDER}}		& \sout{\emph{flag}}	& \sout{no}	&
			\sout{search-order word set present} \\
		\remove{x:wordset-query}{\texttt{SEARCH-ORDER-EXT}}& \sout{\emph{flag}}	& \sout{no}	&
			\sout{search-order extensions word set present} \\
