...
The words \word[legacy]{numTIB}, \word{toIN}, \word{BASE}, \word[block]{BLK},
\word[block]{SCR}, \word{SOURCE}, \word{SOURCE-ID}, \word{STATE}, and
\word[legacy]{TIB} contain information used by the Forth system in its
...
\item \remove{x:legacy}{%
	Words that are being deprecated in favor of new words introduced
	to solve specific problems (e.g., \word[legacy]{CONVERT}).}
\end{itemize}
...
\section{The optional Legacy word set} % A.x =======================
\setwordlist{string}
\label{rat:legacy}

\place{x:legacy}{%
As implementations have become more elaborate and practices have
been superceded, the words that support those practices are now
considered obsolete.  These words were marked as obsolete in the
previous standard.  They have been grouped together into the legacy
word set as although obsolete these words are still to be found in
widespread use in legacy code.}

\place{x:legacy}{%
Their definitions are retained in this word set, so that legacy
code will continue to operate.  However, their use in new
implementations or new program is strongly discouraged.}

\setcounter{subsection}{5}
\subsection{Glossary} % A.xx.6 ============================

\readrationale{legacy}
