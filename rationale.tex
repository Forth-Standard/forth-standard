...
\item \textbf{Standard Character Set}
	...
	\item \remove{x:2-complement}{The allowed number representations are two's-complement,
...
\remove{ed16}{EBCDIC is out (most ``EBCDIC'' computer systems support ASCII too).}
...
\cbstart\patch{x:2-complement}\uline{%
As a consequence of these behaviors, the likely range of
signed numbers is $-n-1$ to $n$ and for
unsigned numbers is $0$ to $2n+1$.
Where $n$ is the largest positive signed number.
...
\begin{center}
  \begin{tabular}{cr@{\sout{ to }}llr@{\sout{ to }}l}
	\hline\hline
	\textbf{\sout{Arithmetic architecture}}
	&\multicolumn{2}{c}{\textbf{\sout{signed numbers}}}
	&\multicolumn{3}{c}{\textbf{\sout{unsigned numbers}}} \\
	\hline
	\sout{Two's complement}	& \sout{$-n-1$} & \sout{$n$}	&& \sout{$0$} & \sout{$2n+1$}	\\
	\sout{One's complement}	& \sout{$-n$} 	& \sout{$n$}	&& \sout{$0$} & \sout{$n$}		\\
	\sout{Signed magnitude}	& \sout{$-n$} 	& \sout{$n$}	&& \sout{$0$} & \sout{$n$}		\\
	\hline\hline
  \end{tabular}
\end{center}

\sout{where $n$ is the largest positive signed number. For all three
...
\begin{quote}
  \begin{tabular}{ll}
	\sout{two's complement:} & \sout{\word{:} \word{NEGATE} \word{INVERT} \word{1+} \word{;}} \\
	\sout{one's complement:} & \sout{\word{:} \word{NEGATE} \word{INVERT} \word{;}} \\
	\sout{signed-magnitude:} & \sout{\word{:} \word{NEGATE} \texttt{HIGH-BIT} \word{XOR} \word{;}}
  \end{tabular}
\end{quote}

\sout{where \texttt{HIGH-BIT} is a bit mask with only the most-significant
bit set. Note that all of these number systems agree on the
representation of non-negative numbers.}

\sout{Per} \xref[3.2.1.1 Internal number representation]{usage:number} \sout{and}
\wref{core:0=}{0=}\sout{, the implementor must ensure that no standard or
...
Whether underflow occurs depends on the data-type of the result.
\replace{x:2-complement}{For
example, the phrase \texttt{1 2} \word{-} underflows if the result is
unsigned and produces the valid signed result -1.}{For
example, the phrase \texttt{1 2} \word{-} underflows and produces
the largest unsigned number if the result is unsigned and produces
the valid signed result \texttt{-1}.}
...
advantage of some assumed architecture.
\remove{x:2-complement}{For example, most computers
...
\place{x:2-complement}{For example, by acknowledging an
environmental dependency on big-endian memory access,
a programmer becomes entitle to use the lower memory address
of a cell as its the most significant byte.}
...
\vspace*{-1ex}\enlargethispage{2ex}
