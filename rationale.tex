...
\remove{F94}{ANS} Forth can be provided for resource-constrained embedded systems.
...
When judging relative merits, the members of the \remove{F94}{X3J14} Technical
...
From the beginning, the \remove{F94}{X3J14} Technical Committee faced not only
...
First, \remove{F94}{ANS} Forth systems can continue to be implemented on a greater
...
hardware will be able to accommodate \replace{F94}{an ANS}{a} Forth implementation.
...
Third, \remove{F94}{ANS} Forth systems may be designed to offer the user the power
...
The \remove{F94}{X3J14} Technical Committee has endeavored to accommodate this
...
\replace{F94}{The trend in ANS Forth is to move}{Forth 94 moved} toward the consistent use of the
...
to the Forth-83 divide.  To resolve this issue, \replace{F94}{an ANS Forth}{a Forth} system
is permitted to supply either floored or symmetric operators. In
addition, \replace{F94}{ANS Forth systems}{a standard system} must provide a floored division primitive
...
compliant with \replace{F94}{ANS Forth}{Forth 94} with respect to division. In practice, the
...
and Forth-83 programs to \replace{F94}{ANS Forth}{Forth 94} without any analysis of the
original programs.
...
The simple implementation of the \replace{F94}{ANS Forth}{Forth 94} \word{CASE} structure
...
\subsection{Extension queries\strike{-2}{90}}

\subsubsection{Obsolescent Environmental Queries} % A.3.2.7
\label{rat:obsolete}

When reviewing the \place{ed11}{Forth 94} Standard\place{ed11}{,} the question of adapting the word set
queries had to be addressed.  Despite the recommendation in the
\replace{ed11}{previous (Forth-94)}{Forth 94} standard, word set queries have not been
...
queries for the revised word sets.  The TC has therefore\remove{ed11}{,} declared
...
next revision.
They are retained in this Standard to support existing \replace{F94}{ANS Forth}{Forth 94}
programs.  New programs should not use them.

\cbstart
\subsubsection{Extension queries\strike{4.5}{115}} % A.3.2.8
\cbend
...
An implementor \remove{F94}{of ANS Forth} can handle these alignment restrictions
...
in some hardware environments.  An alternate implementation \remove{F94}{of ANS Forth}
...
address falls on the programmer.  A portable \remove{F94}{ANS Forth} program must
...
\paragraph{Numeric notation}\strike{4}{140}~
\label{rat:notation:numeric}
% A.3.4.1.3

\remove{ed11}{The numeric representation provided by the \textsf{X:number-prefix}
extension can be tested with the following test cases:}

\begin{tt}
\word{DECIMAL}\strike{4}{48} \\
\test{\#1289      }{1289       }\strike{3}{190} \\
\test{\#12346789. }{12346789.  }\strike{3}{190} \\
\test{\#-1289     }{-1289      }\strike{3}{190}\\
\test{\#-12346789.}{-12346789. }\strike{3}{190} \\
\test{\$12eF      }{4847       }\strike{3}{190} \\
\test{\$12aBcDeF. }{313249263. }\strike{3}{190} \\
\test{\$-12eF     }{-4847      }\strike{3}{190} \\
\test{\$-12AbCdEf.}{-313249263.}\strike{3}{190} \\
\test{\%10010110  }{150        }\strike{3}{190} \\
\test{\%10010110. }{150.       }\strike{3}{190} \\
\test{\%-10010110 }{-150       }\strike{3}{190} \\
\test{\%-10010110.}{-150.      }\strike{3}{190} \\
\test{'z' \       }{122        }\strike{3}{190}
\end{tt}
\cbend
...
\subsection[Forth systms]{ANS\strike{4}{25} Forth systems} % A.5.1

Section \ref{label:label} defines the criteria that a system must
meet in order to justify the label ``\replace{F94}{ANS}{Standard} Forth System''. Briefly,
...
A Forth cross-compiler has many characteristics in common with \replace{F94}{an ANS Forth System}{a standard system},
in that both use similar compiling tools to process a
program. However, in order to fully specify \replace{F94}{an ANS Forth}{a standard} cross compiler
...
for a Forth cross-compiler to correctly prepare \replace{F94}{an ANS Forth}{a standard} program
for execution in a target environment, it is inappropriate for a
cross-compiler to be labeled \replace{F94}{an ANS Forth System}{a standard system}.

\subsection[Forth programgs]{ANS\strike{4}{25} Forth programs} % A.5.2
...
\replace{F94}{an ``ANS Forth Program''}{a ``Standard Forth Program''}
...
implementations, \replace{F94}{ANS Forth}{This standard} requires that the Block word set be
...
\subsection{Additional usage requirements} \cbstart\patch{x:block}
\setcounter{subsubsection}{1}
...
to the \replace{F94}{ANS}{standard} behaviors, and hope that if this is done on a widespread
...
the inclusion and makeup of the Floating-Point Word Sets in \replace{F94}{ANS Forth}{Forth 94}.
Although it has been argued that \replace{F94}{ANS Forth}{the standard} should not address
...
inclusion of Floating-Point in \replace{F94}{ANS Forth}{Forth 94}. There is substantial common
...
\item[Floating-point input:]
	...
	be interpreted by \replace{F94}{an ANS Forth}{a standard} system must contain the exponent
...
\item[Floating-point representation:]
	...
	implementation defined in \replace{F94}{ANS Forth}{this standard}, the Floating-Point Extensions
...
argued forcefully that \replace{F94}{ANS Forth}{Forth 94} should say nothing about locals
since:
...
group that it is hoped will ``broadly accept'' \replace{F94}{ANS Forth}{Forth 94} unless the
...
present the consensus of the Technical Committee that if \replace{F94}{ANS Forth}{Forth 94} has
...
\item[i)] \begin{quote}\ttfamily
	...
	\tab[0.45] \word{[} \word{'} A \word{]} \word{LITERAL} LEE
	\word{;}
	\end{quote}
\end{enumerate}
...
\replace{F94}{ANS Forth has}{Forth 94} removed the word \texttt{CONTEXT} because in many systems
