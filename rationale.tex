...
The other\place{x:wordset-query}{, now obsolescent,} group of queries
...
to change \word{ENVIRONMENTq}'s database so that subsequent queries about
...
\subsection{Obsolescent Environmental Queries\strike{-2}{225}}
\label{rat:obsolet}
...
The words \remove{x:legacy}{\word{numTIB},} \word{toIN}, \word{BASE}, \word[block]{BLK},
\word[block]{SCR}, \word{SOURCE}, \word{SOURCE-ID}, \word{STATE}%
\remove{x:legacy}{, and and \word{TIB}} contain information used by the Forth system in its
...
\item Words that are being deprecated in favor of new words introduced
	to solve specific problems \remove{x:legacy}{(e.g., \word{CONVERT})}.
\end{itemize}
...
char sets have to be used, several of them variable-width.  Currently,
the most popular representative of the variable-width character sets
is UTF-8.}
