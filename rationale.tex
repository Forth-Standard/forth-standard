...
\place{ed13}{of proposed changes to the standard}, the members of the
...
\cbstart\patch{ed13}
\subsection[Document organization]{\sout{Document organization}} % A.1.3
\label{doc:org}
\cbend
...
\paragraph{\uline{Error results}} ~ % A.3.1.3.6
\label{rat:ior}
...
etc.) could be implemented in terms of smaller-width access instructions\place{ed13}{,}
...
The data space of a Forth system comes in \replace{ed13}{discontinuous}{ discontiguous} regions. The
...
\remove{ed13}{%
	Annex \textbf{C.7.9 Multitasking} describes the topic in more detail.
}
