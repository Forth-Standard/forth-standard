...
\replace{F94}{ANS Forth}{Forth 94} can be provided for resource-constrained embedded systems.
...
First, \replace{F94}{ANS Forth}{Forth 94} systems can continue to be implemented on a greater
...
hardware will be able to accommodate an \replace{F94}{ANS Forth}{Forth 94} implementation.
...
Third, \replace{F94}{ANS Forth}{Forth 94} systems may be designed to offer the user the power
...
The trend in \replace{F94}{ANS Forth}{Forth 94} is to move toward the consistent use of the
...
to the Forth-83 divide.  To resolve this issue, an \replace{F94}{ANS Forth}{Forth 94} system
is permitted to supply either floored or symmetric operators. In
addition, \replace{F94}{ANS Forth}{Forth 94} systems must provide a floored division primitive
...
An implementor of \replace{F94}{ANS Forth}{Forth 94} can handle these alignment restrictions
...
in some hardware environments. An alternate implementation of \replace{F94}{ANS Forth}{Forth 94}
...
address falls on the programmer. A portable \replace{F94}{ANS Forth}{Forth 94} program must
...
A Forth cross-compiler has many characteristics in common with an \replace{F94}{ANS Forth}{Forth 94}
System, in that both use similar compiling tools to process a
program. However, in order to fully specify an \replace{F94}{ANS Forth}{Forth 94} cross compiler
...
for a Forth cross-compiler to correctly prepare an \replace{F94}{ANS Forth}{Forth 94} program
for execution in a target environment, it is inappropriate for a
cross-compiler to be labeled an \replace{F94}{ANS Forth}{Forth 94} System.
...
implementations, \replace{F94}{ANS Forth}{Forth 94} requires that the Block word set be
...
to the \replace{F94}{ANS}{Forth 94} behaviors, and hope that if this is done on a widespread
...
Although it has been argued that \replace{F94}{ANS Forth}{Forth 94} should not address
...
\item[Floating-point input:]
	...
	be interpreted by an \replace{F94}{ANS Forth}{Forth 94} system must contain the exponent
...
\item[Floating-point representation:]
	...
	implementation defined in \replace{F94}{ANS Forth}{Forth 94}, the Floating-Point Extensions
...
\replace{F94}{ANS Forth}{Forth 94} has removed the word \texttt{CONTEXT} because in many systems
