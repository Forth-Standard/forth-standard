...
\place{X:structures}{Append table \ref{facility:types} to table \ref{table:datatypes}.}
...
\begin{table}[h]
  \begin{center}
	...
	\hline
	\place{X:strutures}{\emph{struct-sys}}
	& \place{X:strutures}{data structures}
	& \place{X:strutures}{implementation dependent} \\
	\hline\hline
	\end{tabular}
  \end{center}
\end{table}
...
\place{X:structures}{%
The implementation-dependent data generated and upon beginning to
compile a data structure, with \word{BEGIN-STRUCTURE}, and consumed
at its close, with \word{END-STRUCTURE}, is represented by the
symbol \param{struct-sys} thoughout this Standard.}
...
\item \place{X:structures}{%
	A \param{name} defined by
...
\see \wref{facility:+FIELD}{+FIELD} and
	...
	\begin{defer}
	\rationale
		There are two schools of though regarding named data
		...
		\tab 1 \word{CELLS} \word{+FIELD} p.y   \tab \word{bs} A signle cell field named p.y \\
		\word{CONSTANT} point \tab[3.8] \word{bs} save strucutre size
		\end{quote}
		...
		\begin{quote}\ttfamily
		\word{BEGIN-STRUCTURE} point \tab[-.3] \word{bs} create the named strucutre \\
		\tab 1 \word{CELLS} \word{+FIELD} p.x	\tab \word{bs} A signle cell filed named p.x \\
		\tab 1 \word{CELLS} \word{+FIELD} p.y   \tab \word{bs} A signle cell field named p.y \\
		\word{END-STRUCTURE}
		\end{quote}
		...
		as the \param{struct-sys} paramater, while the name last
		school would defined \param{struct-sys} as being empty.

		Executing the name of the data structure, retuns the size of
		the data strcutre.  This allows the data strcture to be used
	...
	\item[Alignment]
		...
		second.  This leads to various customised solutions, usage
		...
		The standard currently defines an aligned filed defining
		...
		data types. Note that these also satisfy the alignment
		requirements of the host system, whereas \word{+FIELD}
		does not.
...
\see \wref{facility:KEYq}{KEY?},
	...
	\rationale % A.10.6.2.1305 EKEY
		...
		\place{ed07}{A standard program may only interpret the
		results of \word{EKEY} via the translation words provided
		for that purpose (\word{EKEYtoCHAR} and \word{EKEYroFKEY}).}
