...
\see \wref{facility:EKEY}{EKEY},
	...
	\rationale
		...
		The \replace{ed}{following}{\texttt{K-}{\ldots}} words produce
		the same values that \place{ed}{the sequence} \word{EKEY}
		\word{EKEYtoFKEY} may produce when the user presses the
		corresponding keys}. Note that\replace{ed}{, even
		if this extension is present, the keyboard may}{ some systems
		may} lack some of the keys, or the system the capability to
		report them\replace{ed}{, so programs}{. Programs} should be written
		such that they \replace{ed}{also}{continue to} work (although
		less conveniently or with less functionality) if these key
		\replace{ed}{numbers}{combinations} cannot be produced.
	...
	\item[\word{EKEY} return values] ~\\
		...
		implementation defined keyboard scan code.
		\remove{ed}{Unfortunately there are some systems where the scan
		code is larger than a single cell.}
		The implementation defined nature of the value means that a
		standard program may not use the value \replace{ae}{is}{in} any way, other than
		passing it on to \word{EKEYtoCHAR} to convert it into an ASCII
		character.

		\word{EKEYtoFKEY} has been designed to be used in a similar
		manner. It provides \replace{ed}{a}{an} implementation defined
		value which corresponds to \place{ed}{non-ASCII keys, or} a
		combination \replace{ed}{of keys}{thereof}. A standard program
		\replace{ed}{can}{may}
		check which key combination has been used via a set of
		pre-defined constants.

		\remove{ae}{%
		\replace{ed}{A system where the scan code is larger than a
		single cell}{There are some systems where the scan code is
		larger than a single cell, such systems} may
		approach this in one of two ways. It may manipulate the scan
		code \place{ed}{returned from \word{EKEY}} to produce a single
		cell value.  Alternatively it may return the address of a
		buffer which contains the actual scan code.}
...
%		Programming advice
		...
		\remove{ed}{%
		Note that many environments do not have or do not report all
		these keys and key combinations, so it is a good idea to write
		a program such that it is still useful even if these keys and
		key combinations cannot be pressed or are not recognised.}
	...
	\item[Shift keys] ~\\
		...
		keys, because these are the only
		\replace{ed}{keys that \word{EKEY} return values are defined
		for.}{values defined for use with \word{EKEYtoFKEY}.}
