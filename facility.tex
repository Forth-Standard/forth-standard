...
\subsection{Environmental queries\strike{2.4}{160}} % 10.3.2
\cbstart\patch{x:wordset-query}%

\sout{Append table \ref{facility:env} to table \ref{table:env}.}
...
\begin{table}[h]
  \begin{center}
	...
	\begin{tabular}{p{9em}rcp{0.42\textwidth}}
		...
		\hline
		\sout{\texttt{FACILITY}}		& \sout{\emph{flag}}	& \sout{no}	&
			\sout{facility word set present} \\
		\sout{\texttt{FACILITY-EXT}}	& \sout{\emph{flag}}	& \sout{no}	&
			\sout{facility extensions word set present} \\
		\hline\hline
	\end{tabular}
  \end{center}
\end{table}
...
\item \stack{}{}\strike{2.5}{35} \stack{n_1 "<spaces>name"}{n_2}\strike{-3}{120}

	\sout{%
		The semantics of \word{CFIELD:} are identical to the
		execution semantics of the phrase:}
	\begin{quote}\ttfamily
	\sout{1 \word{CHARS} \word{+FIELD}}
	\end{quote}

	\uline{Skip leading space delimiters. Parse \param{name} delimited by
	a space. \param{Offset} is the first character aligned value greater
	than or equal to}
	\param{n_1}\strike{-2.6}{11}.
	\param{n_2 = offset + 1}\strike{-2.6}{60} \uline{character.}

	\uline{Create a definition for \param{name} with the execution semantics
	given below.}

\execute[name] \stack{a-addr_1}{a-addr_2}\strike{-3}{170}

	\uline{Add the \param{offset} calculated during the compile time action
	to}
	\param{a-addr_1}\strike{-2.6}{34}
	\uline{giving the character aligned address}
	\param{a-addr_2}\strike{-2.6}{34}.


\see \wref{facility:+FIELD}{+FIELD},
	\wref{facility:BEGIN-STRUCTURE}{BEGIN-STRUCTURE},
	\wref{facility:END-STRUCTURE}{END-STRUCTURE}%
	\uline{, \\ \rref{facility:FIELD:}{}}.

	\begin{implement} % I.10.6.2.---- CFIELD
		\sout{\textdf{Create a new field within a structure definition
		of size a character.}}

		\sout{\word{:} CFIELD:\tab\word{bs} n1 <"name"> -{}- n2 ; Exec: addr -{}- 'addr} \\
		\tab \sout{1 \word{CHARS} \word{+FIELD}} \\
		\sout{\word{;}}
	\end{implement}
\end{worddef*}
...
\item \stack{}{\replace{x:ekey}{u}{x}}

	Receive one keyboard event \param{\replace{x:ekey}{u}{x}}. The encoding of keyboard events
...
\item \stack{\replace{x:ekey}{u}{x}}{\replace{x:ekey}{u}{x} false | char true}

	If the keyboard event \param{\replace{x:ekey}{u}{x}} corresponds to a character in the
	implementation-defined character set, return that character and
	\emph{true}. Otherwise return \param{\replace{x:ekey}{u}{x}} and \emph{false}.
...
\item \stack{\replace{x:ekey}{u_1}{x}}{\replace{x:ekey}{u_2 f}{ u flag}}

	If the keyboard event \param{\replace{x:ekey}{u_1}{x}} corresponds to a keypress in the
	implementation-defined special key set, return that key's id
	\param{\replace{x:ekey}{u_2}{u}} and \param{true}. Otherwise return \param{\replace{x:ekey}{u_1}{x}} and
	\param{false}.
...
\item \stack{}{}\strike{2.5}{35} \stack{n_1 "<spaces>name"}{n_2}\strike{-3}{120}
	...
	\sout{%
		...
		execution semantics of the phrase:}
	\begin{quote}\ttfamily
	\sout{\word{ALIGNED} 1 \word{CELLS} \word{+FIELD}}
	\end{quote}
	...
	than or equal to}
	\param{n_1}\strike{-2.6}{11}.
	\param{n_2 = offset + 1}\strike{-2.6}{60} \uline{cell.}
...
\execute[name] \stack{a-addr_1}{a-addr_2}\strike{-3}{170}
	...
	to}
	\param{a-addr_1}\strike{-2.6}{34}
	\uline{giving the cell aligned address}
	\param{a-addr_2}\strike{-2.6}{34}.
...
\see \wref{facility:+FIELD}{+FIELD},
	...
	\begin{rationale}
		\uline{Create an aligned single cell field in a data structure.}

		\uline{The various \emph{x}\texttt{FIELD:} words provide for different
		alignment and size allocation.}

		\uline{The \emph{x}\texttt{FIELD:} words could be defined as:}

		\begin{tabbing}
			\tab \= \tab[7] \= \tab[5] \= \tab[6] \= \tab[5.2] \= \kill
			\> \word{:} \word{FIELD:}   \>\> \word{ALIGNED}   \' 1 \word{CELLS}   \> \word{+FIELD} \word{;} \> \word{bs} cell aligned
	\strike{-2.5}{297}\\
			\> \word{:} \word{CFIELD:}  \>\>                  \' 1 \word{CHARS}   \> \word{+FIELD} \word{;} \> \word{bs} character aligned
	\strike{-2.5}{320}
		\setwordlist{floating} \\
			\> \word{:} \word{FFIELD:}  \>\> \word{FALIGNED}  \' 1 \word{FLOATS}  \> \word[facility]{+FIELD} \word{;} \> \word{bs} float aligned
	\strike{-2.5}{300} \\
			\> \word{:} \word{SFFIELD:} \>\> \word{SFALIGNED} \' 1 \word{SFLOATS} \> \word[facility]{+FIELD} \word{;} \> \word{bs} single-float aligned
	\strike{-2.5}{326} \\
			\> \word{:} \word{DFFIELD:} \>\> \word{DFALIGNED} \' 1 \word{DFLOATS} \> \word[facility]{+FIELD} \word{;} \> \word{bs} double-float aligned
	\strike{-2.5}{329.5}
	...
	\begin{implement} % I.10.6.2.---- FIELD:
		...
		\sout{\word{:} field:\tab\word{bs} n1 <"name"> -{}- n2 ; Exec: addr -{}- 'addr} \\
		\tab \sout{\word{ALIGNED} 1 \word{CELLS} \word{+FIELD}} \\
		\sout{\word{;}}
	\end{implement}
\end{worddef*}
