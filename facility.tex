...
\see \place{ed09b}{\rref{facility:AT-XY}{}.}
...
\see \place{ed09b}{\rref{facility:KEYq}{}.}
	...
	\begin{rationale} % A.10.6.1.1755 KEY?
		...
		application does not wish to
\replace{ed09b}{bother with}{process}
...
\see \wref{facility:BEGIN-STRUCTURE}{BEGIN-STRUCTURE},
	...
	\wref{floating:SFFIELD:}{SFFIELD:} \remove{ed09b}{and}
	\wref{floating:DFFIELD:}{DFFIELD:}%
	\place{ed09b}{, \rref{facility:+FIELD}{}}.
...
\see \wref{facility:+FIELD}{+FIELD}\replace{ed10}{ and}{,}
	\wref{facility:END-STRUCTURE}{END-STRUCTURE}%
	\place{ed09b}{, \rref{facility:BEGIN-STRUCTURE}{}}.
...
\see \wref{facility:+FIELD}{+FIELD},
	\wref{facility:BEGIN-STRUCTURE}{BEGIN-STRUCTURE}, \remove{ed09b}{and}
	\wref{facility:END-STRUCTURE}{END-STRUCTURE}.
...
\see \wref{facility:KEYq}{KEY?},
	\wref{core:KEY}{KEY}%
	\place{ed09b}{, \rref{facility:EKEY}{}.}


	\begin{rationale} % A.10.6.2.1305 EKEY
\cbstart\patch{x:key-ekey}\uline{%
		...
		system-\linebreak dependent set of events.}
	...
	\sout{%
		...
		\begin{quote}
\sout{The \texttt{MAX-CHAR} environmental query would return 255.}

\sout{Assume the existence of a word \texttt{DOS-KEY}}
\stack{}{char}\strike[6]{2.2}{60}
\sout{which executes the MS-DOS ``Direct STDIN Input'' system call
			(Interrupt 21h, Function 07h) and a word
			\texttt{DOS-KEY?}}
\stack{}{flag}\strike[6]{2.2}{60}
\sout{which executes the MS-DOS ``Check STDIN Status'' system call
			(Interrupt 21h, Function 0Bh).}

			\ttfamily
			\begin{tabbing}
			\tab \= \tab \= \tab \= \tab \= \hspace{9em} \= \kill
    \sout{\word{:} \word{EKEYq}~ \word{p} -{}- flag )~
				DOS-KEY?~ \word{0ne} \word{;}} \\[\parskip]

    \sout{\word{:} \word{EKEY}~ \word{p} -{}- u )~
				DOS-KEY~ \word{qDUP} \word{0=} \word{IF}~
					DOS-KEY 256 \word{+}~ \word{THEN} \word{;}} \\[\parskip]

\+ \sout{\word{:} \word{EKEYtoCHAR} \word{p} u -{}- u false | char true )} \\
\+ \sout{\word{DUP} 255 \word{more} \word{IF}} 		\>\>\>\>	\sout{\word{p} u )} \\
   \sout{\word{DUP} 259 \word{=} \word{IF}}	 		\>\>\>		\sout{\word{bs} 259 is Ctrl-@ (ASCII NUL)} \\
\> \sout{\word{DROP} 0 \word{TRUE} \word{EXIT}}		\>\>		\sout{\word{bs} so replace with character} \\
\- \sout{\word{THEN} \word{FALSE} \word{EXIT}}		\>\>\>		\sout{\word{bs} otherwise extended character} \\
\- \sout{\word{THEN} \word{TRUE}}					\>\>\>\>	\sout{\word{bs} normal extended ASCII char.} \\
   \sout{\word{;}} \\[\parskip]

   \sout{\word{VARIABLE} PENDING-CHAR ~ -1 PENDING-CHAR \word{!}} \\[\parskip]

\+ \sout{\word{:} \word{KEYq}~ \word{p} -{}- flag )} \\
\+ \sout{PENDING-CHAR \word{@} \word{0less} \word{IF}} \\
\+ \sout{\word{BEGIN}~ \word{EKEYq} \word{WHILE}} \\
\+ \sout{\word{EKEY} \word{EKEYtoCHAR} \word{IF}} \\
\- \sout{PENDING-CHAR \word{!}~ \word{TRUE} \word{EXIT}} \\
\- \sout{\word{THEN} \word{DROP}} \\
\- \sout{\word{REPEAT}~ \word{FALSE} \word{EXIT}} \\
\- \sout{\word{THEN}~ \word{TRUE}} \\
   \sout{\word{;}} \\[\parskip]

\+ \sout{\word{:} \word{KEY}~ \word{p} -{}- char )} \\
\+ \sout{PENDING-CHAR \word{@} \word{0less} \word{IF}} \\
   \sout{\word{BEGIN}~ \word{EKEY}~ \word{EKEYtoCHAR} \word{0=} \word{WHILE}} \\
\> \sout{\word{DROP}} \\
\- \sout{\word{REPEAT}~ \word{EXIT}} \\
\- \sout{\word{THEN}~ PENDING-CHAR \word{@}~ -1 PENDING-CHAR \word{!}} \\
   \sout{\word{;}}
			\end{tabbing}
		\end{quote}
...
\+ \sout{\word{:} \word{EKEY}~ \word{p} -{}- u )} \\
\+ \sout{DOS-KEY-FUNCTION-8~ \word{qDUP}~ \word{0=}~ \word{IF}} \\
   \sout{DOS-KEY-FUNCTION-7~ \word{DUP} 3~ \word{=}~ \word{IF}} \\
\> \sout{\word{DROP} 0~ \word{ELSE}~ 256 \word{+}} \\
\- \sout{\word{THEN}} \\
\- \sout{\word{THEN}} \\
   \sout{\word{;}}
			\end{tabbing}
	...
	\sout{%
		...
		\begin{enumerate}
		\item \sout{%
			An application program that uses \word{KEY},
		...
		\item \sout{%
			An application program that wishes to handle non-character
...
\see \place{ed09b}{\rref{facility:EKEYtoCHAR}{}.}
...
\see \wref{facility:EKEY}{EKEY},
	...
	\wref{facility:K-F11}{K-F11}, \remove{ed09b}{and}
	\wref{facility:K-F12}{K-F12}%
	\place{ed09b}{, \rref{facility:EKEYtoFKEY}{}.}
...
\see \place{ed09b}{\rref{facility:EMITq}{}.}
...
\see \place{ed09b}{\rref{facility:MS}{}.}
...
\see \place{ed09b}{\rref{facility:TIMEandDATE}{}.}
