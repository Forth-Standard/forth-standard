...
\subsection{Environmental queries\strike{2.4}{160}} % 14.3.2
\cbstart\patch{x:wordset-query}%

\sout{Append table \ref{mem:env} to table \ref{table:env}.}
...
\begin{table}[ht]
  \begin{center}
	...
	\begin{tabular}{p{10em}rcp{0.42\textwidth}}
		...
		\hline
		\sout{\texttt{MEMORY-ALLOC}}		& \sout{\emph{flag}}		& \sout{no}	&
			\sout{memory-allocation word set present} \\
		\sout{\texttt{MEMORY-ALLOC-EXT}}	& \sout{\emph{flag}}		& \sout{no}	&
			\sout{memory-allocation extensions word set present} \\
		\hline\hline
	\end{tabular}
  \end{center}
\end{table}
