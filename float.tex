...
Floating-point stack notation \remove{X:fp-stack}{when the floating-point stack is
...
The width of the floating-point stack is
implementation-defined. \remove{X:fp-stack}{By default} The
floating-point stack shall be separate from the data and return
stacks. \remove{X:fp-stack}{A program may determine whether
floating-point numbers are kept on the data stack by passing the
string ``\texttt{FLOATING-STACK}'' to \word[core]{ENVIRONMENTq}.}
...
%\subsubsection{\place{X:fp-stack}{Environmental restrictions}} % 12.4.1.4
...
\item \place{X:fp-stack}{requiring floating-point numbers to be kept
	on dat data stack, with $n$ cells per floating point number.}
\end{itemize}
